...
//
// **FOUR CALLERS.** Two of them cross the boundary through
...
//   3. Start, when Find Overset is ON  -- KCMStartComparisonFor (KCMComparisonRun.cpp). Re-binds
//                                         the scan to the comparison Target so marks and overset
//                                         share one Prev/Next cycle.
//   4. Stop, when Find Overset is ON   -- KCMStopComparison (KCMComparisonRun.cpp). The ONE caller
//                                         that hands over a saved pointer (sOversetDB) rather than
//                                         a freshly resolved one; see the liveness test at the
//                                         head of the .cpp, which exists for this caller.
// @warning this list once read "Find Overset, Refresh while armed, and Start": Stop was missing
//  although the .cpp singles it out by name, and Refresh needs no armed comparison -- two
//  descriptions of one thing, kept in two places, disagreeing ([[one-question-one-place]]).
//
...
//  @warning those three are on the UI side and reach this through
//  IKCMCompareFacade::GetOversetScanTargetDB(), so this spelling appears in exactly one place
//  besides the definition -- the facade forwarder. **Grep BOTH names**, or the callers look like
//  none at all (the same trap KCMBaseScreenOpacity sets).
