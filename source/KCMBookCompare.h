...
//  ⚠It is not true that this touches none of KCMDrawEventHandler's statics, which is what this
...
//   refresh=kTrue) - and every chapter close empties it through KCMHandleDocsClosed, so the
...
/** The last comparison as one block of text: one line per chapter, "name<TAB>state", and a failed
    ...
    ***** Kept in the module, so it answers whether or not a panel is open. ***** Same reason
    app.kcmStatus keeps its line in KCMCore rather than in the panel widget: a comparison run
...
/** Shutdown only: empty the stored text, so the module's static PMString has no live heap buffer to
    ...
    ★Same contract, and the same one-line body, as KCMClearSessionStatus and
    KCMStoryList::ShutdownCleanup. Added 2026-08-18 (bug recheck B8): the shutdown service lists
