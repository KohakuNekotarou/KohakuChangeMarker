...
//
//  See KCMStoryMarkBuild.h for what this shows and why, and ui/KCMStoryPressMarks.h for the
...
#include "KCMCore.h"					// KCMArmedTargetDB / KCMArmedSourceDB (⚠declared here, not in KCMPeek.h)
...
//     down - the double click does exactly that (KCMStorySelectChange), after which this file
...
void KCMStoryCollectRanges(IDataBase* db, bool16 useSourceDocument, KCMStoryMarkMap& out)
{
	...
	// ⚠ASKED WITHOUT A nil TEST, WHERE KCMStoryMarkRefresh BELOW TESTS ITS OWN FACADE - and the
	...
	for (int32 n = 0; n < rowCount; ++n)
	{
		...
		if (changeCount <= 0)
		{
			...
			//   (KCMStoryStamp.h, kKCMStoryKindUnpaired). Every character of them is new, or
			...
			//   in the second (IKCMStoryEditsFacade.h names all three).
			...
			//   settled by the row filter above (KCMStoryStamp.h:117-127).
		...
		for (int32 i = 0; i < changeCount; ++i)		// no changes = not entered; the whole range is already in
		{
			...
			if (to == from)
			{
				...
				//     other one; the flag is all the drawing side needs (KCMStoryMarkRanges.h).
...
void KCMStoryMarkRefresh()
{
	...
	if (compare != nil && compare->IsArmed() && compare->GetCompareMode() == kKCMModeStory)
	{
		...
		//   it. KCMStoryMarker asks for itself now, at the moment it draws.
		...
		//   permanently as well (KCMDrawEventHandler.cpp:2093). ⇒ WYSIWYG: what will come out is
		...
		//   its screen and its paper, which is the specification in IKCMCompareFacade.h:146-147.
	...
	if (docs.empty())
	{
		...
		//   because the call names which of the two kinds is coming down (KCMStoryMarker.h).
...
bool16 KCMStoryMarkPrintAllowedFor(IDataBase* db)
{
	...
	//   two flags the same way (KCMRingAdornment.cpp:253).
	// ⚠KCMIsSameDoc, NOT ==: an asynchronous export hands the drawing a CLONE of the database, so
	//   the pointer never matches the one we armed with (KCMThreadSafety.h has the measurement).
	...
	//   IKCMCompareFacade.h:146-147 long before the Story mode could print at all.
