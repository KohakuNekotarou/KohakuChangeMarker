...
//    - KCMElidePathFront. Shortening a path so it fits is the view's decision, and all three
//      callers were UI-side, so it moved to KCMBookDialog.h in this same task instead of
...
//  (ui/KCMBookPanelLookup.h's KCMGetPanelBookFile) and hands the file in, which is what the
...
//  (found 2026-08-18, bug recheck B8), and the stale one pointed at KCMBookPair.cpp:186-249,
...
// Project includes:
#include "KCMBoundaryID.h"	// IID_IKCMBOOKFACADE。★2026-08-17 に KCMID.h から絞った
								// (理由は IKCMCompareFacade.h の同じ位置)
#include "KCMBookResult.h"	// KCMChapterResult / KCMChapterState -- plain data and one
...
public:
	...
	/** Which two books a comparison would run on: Target is the book in `panelBookFile`, Source is
		...
		ui/KCMBookPanelLookup.h's KCMGetPanelBookFile, and hands the answer across.
	...
	/** The book's FULL PATH as the file system spells it, falling back to its title when it has no
		...
		(KCMBookDialog.h's KCMElidePathFront)" until 2026-08-18 (bug recheck B-U5), naming a
		...
		spelled (ui/KCMPathDisplay.h) -- and the one place with no widget, the confirmation
	...
	/** Run the comparison. Raises a progress bar naming the chapter being examined, and that bar
		...
		@return ★kSuccess, ALWAYS - and the only caller ignores it on purpose. A chapter that could
			...
			change KCMBookRun.cpp, which today stores and shows the result without asking.
