...
REGISTER_PMINTERFACE(KCMAfterOpenDocResponder, kKCMAfterOpenResponderImpl)	// Puts back the ticks and paws an opened document carries, and attaches the marks observer to it.
REGISTER_PMINTERFACE(KCMPageMarksCmd, kKCMSetPageMarksCmdImpl)	// The one command that writes a tick or a cat paw into the document (KCMPageMarksCmd.cpp). Nothing else may write those labels; it is what makes the change undoable.
REGISTER_PMINTERFACE(KCMResourceSaxHandler, kKCMResourceSaxHandlerImpl)	// Reads the Resources mode's exported XML, keeping one item per definition (KCMResourceParse.cpp). It registers no element names, which is what leaves it the DEFAULT handler and lets the mode work off a blacklist.
REGISTER_PMINTERFACE(KCMResourceSink, kKCMResourceSinkImpl)	// Where that handler writes, on the same boss (same file). Separate from the handler because a boss builds one object per IID.
REGISTER_PMINTERFACE(KCMPageMarksCmdData, kKCMPageMarksCmdDataImpl)	// Its parameter interface on the same boss (same file). Non-persistent: what the command was asked to do is not document data.
REGISTER_PMINTERFACE(KCMMarksObserver, kKCMMarksObserverImpl)	// The lazy observer AddIn'd on kDocBoss (KCMMarksObserver.cpp). The ONLY writer of the session stores of ticks and paws - it refills them from the labels, on the way in and again on undo and redo.
REGISTER_PMINTERFACE(KCMStoryUndoObserver, kKCMStoryUndoObserverImpl)	// ★The lazy observer AddIn'd on kTextStoryBoss (KCMStoryUndoObserver.cpp, 2026-09-15): one KCMNotify when a compared story's text changes, so the panel follows a Ctrl+Z. It writes nothing - whether a change is drawn as taken in is derived from the story's counter, which the undo has already put back.
...
REGISTER_PMINTERFACE(KCMScriptProvider, kKCMScriptProviderImpl)	// Serves every published property on its own - on app, story and document (the list is KCM.fr's Provider blocks; it is not repeated here because two earlier copies of it went stale). No methods. A ScriptProvider is not UI, which is why it is on this side.
...
REGISTER_PMINTERFACE(KCMResourcesFacade, kKCMResourcesFacadeImpl)	// How the UI reads which DEFINITIONS differ - the Resources mode (same AddIn; read-only apart from Compare). ⚠It was written and AddIn'd without this line, and the failure looked nothing like a missing registration: the .fr AddIn IS the registration table, so the boss listed the IID and inspect_ui showed it, while Utils<> and CreateObject2 both came back nil. The table says what an object HAS; this list is what can be BUILT.
