...
//  KCMOversetScan.cpp
//
//  Find Overset の検出本体(KCMOversetScan.h 参照)。アクティブ1文書の全ユーザーストーリーを走査し、
...
//  変換は KCMFramePageUID(KCMCore.cpp)に任せる。ページに載らないフレームには kInvalidUID が返る
//  ので、ペーストボードは自然に脱落する。
//  ⚠★**その中身をここで説明しない**——2026-08-09 に KCMCore.cpp へ移した関数で、どの API を主に
...
//          文字列**」の3つ組だけ(IPreflightFacade.h:528-537)。KCM が要るのは「+」点のペーストボード
...
//   KCMFramePageUID が使っていたもので、あの関数は 2026-08-09 に KCMCore.cpp へ移っている
...
// プロジェクト内:
#include "KCMCore.h"			// KCMFramePageUID(フレーム→ページ。Story Edits と共有する1本)
...
//
// ★下の KCMLastPlacedOutport と同じ walk だが、あちらは「+」を描く位置(幾何)まで作り、こちらは
...
//
// ⚠★**登り(下のループ)には KCM の2つの呼び手からは到達しない**(2026-08-17 に数えた)。**残す**が、
//   「効いている」と読まないこと:
//     ・セル経路 …… 呼ぶ直前に KCMThreadHasPlacedParcel が真を確かめている＝押し出されたセルは
//                    そこで弾かれるので、最初の KCMLastPlacedOutport が必ず答える
...
// ★フレーム UID → ページ UID の KCMFramePageUID は KCMCore.cpp へ移した(2026-08-09)。
//   Story Edits の一覧が「ストーリーの先頭フレームはどのページか」を同じ問いとして必要としたため。
//   ここに残して向こうへ写すと、同じプラグインの中に同じ処理が2つ並ぶ——KCM が過去に何度も
//   踏んだ「割れ」の形なので、写さずに1本を共有する。宣言は KCMCore.h。
...
// （SnpIterTableUseDictHier.cpp:247-262 が公式の手本）で取り、KCMThreadIsOverset で判定→あふれなら
// KCMFindOversetOutport+KCMFramePageUID で位置追加。
...
//       test」)。2026-08-17 に KCM 側にも書き足した＝**移植で落ちていた但し書き**。
	...
	for (UID nextUID = storyRef.GetUID(); nextUID != kInvalidUID; nextUID = dictHier->NextUID(nextUID))
	{
		...
		for (ITableModel::const_iterator it(tableModel->begin()), end(tableModel->end()); it != end; ++it)
		{
			...
			if (!KCMThreadIsOverset(textModel, cellPos))
				continue;
			// ★★このセルは「溢れた」のか「組まれる機会が無かった」のか(上の KCMThreadHasPlacedParcel)。
			//   表がフレームごと押し出されると中の全セルが GetIsOverset に yes と答えるので、ガードが
			//   無いと 4行×2列の表で 8 個の「あふれ」を報告してしまう。しかも下の
			//   KCMFindOversetOutport が祖先へ登るため、**位置は 8 個とも親フレームの同じ「+」点**＝
...
//========================================================================================
// KCMCollectOversetLocations(KCMOversetScan.h で宣言)
//========================================================================================
void KCMCollectOversetLocations(IDataBase* db, std::vector<KCMOversetLoc>& outLocs)
{
	...
	for (int32 i = 0; i < n; ++i)
	{
		...
		//   KBSJump.cpp:161-163 が理由・:175-179 が実装)が、KCM に届いていなかった。
		...
		//     の damage を自分では解消しない。
		//   ⚠この関数の呼び手は KCMApplyOversetForDoc ただ1つだが、**その Apply の呼び手は4つある**
		//     (Find Overset / Refresh Overset / Start / Stop。全数は KCMOversetApply.h)。
		//     ★**「描画イベント中には走らない」は、その4つを数えて初めて言える。** 4つとも数えた＝UI の
		//     メニュー2つ／Start は KCMDoMarkChangesDoc が全ページのラスタ化を終えた後／Stop はマーク
		...
		if (frameList != nil && Utils<ITextUtils>()->IsOverset(frameList))
		{
			...
			if (span > 0)
			{
				...
				if (KCMFindOversetOutport(textModel, db, span - 1, frameUID, pb))
				{
					const UID pageUID = KCMFramePageUID(db, frameUID);
					if (pageUID != kInvalidUID)
						outLocs.push_back(KCMOversetLoc(pageUID, pb));	// kInvalidUID はペーストボードのみ=スキップ
				}
			}
		}
