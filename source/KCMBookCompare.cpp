...
#include "KCMConstants.h"			// kKCMResolution / kKCMHiResMul / kKCMCmykThr
...
bool16 OpenChapter(const IDFile& file, UIDRef& outDocRef, bool16& outWeOpened, PMString& outWhy)
{
	...
	if (wantedPath.empty())
	{
		...
		// knows. See KCMBookResult.h's fWhy for the rule and for what it costs when it is broken.
...
/* The folio-exclusion row test (KCMXInRowRects) used to be copied here, because the document
   comparison's copy is file-static and could not be reached. The two were a pair held together by
   a comment. 2026-08-18: the single definition now lives in KCMDrawEventHandler.h as an inline
                            ...
                            std::vector<KCMChapterResult>& outChapters, PMString& outReport)
{
	...
	for (size_t i = 0; i < outChapters.size(); ++i)
	{
		    ...
		    !OpenChapter(chapter.fSourceFile, sourceRef, sourceMine, why))
		{
			...
			// (see KCMChapterStatusText), and it is asked ONLY here, on the failing path, so a
	...
	// kKCMChapterUnknown is the internal "not judged yet" and must not reach the screen; leaving it
