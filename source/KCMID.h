...
//   ㊵★★**Story を Word(.docx)で往復する**(2026-09-19 ユーザー設計。設計書＝`docs/superpowers/specs/2026-09-19-kcm-story-docx-roundtrip-design.md`)。フライアウト **「Export Story Text as Word...」**(ActionID +79・選択の規則は Export Story Text と共通)が 1ストーリー＝1 `.docx` を書き、**Word の変更履歴の記録を ON・「変更履歴のみ許可」で保護**(パスワードなし)して渡す。ルビ＝`w:ruby`／圏点＝文字スタイル `kenten-<種類>`(組み込み10種は使っていなくても styles.xml に載る＝Word のスタイル一覧から付けられる)／縦中横・割注＝`w:eastAsianLayout`／強制改行＝`w:br`／特殊文字＝ロックしたコンテンツコントロール(`w:tag` が真実)／脚注＝`footnotes.xml`(参照位置を運ぶ)／表＝入れ子・結合・ヘッダー行。**書き出しは自分で検算する**(Write→Read→Same を両面で)。**「Import Story Text...」は `.docx` も受ける**(開くダイアログに Word document・組み合わせて選んでよい)＝タグ(`customXml/item1.xml`＝UID・文書名・指紋)で対応づけ、ファイル名の数字がタグと食い違えば断る(流用の事故)。段2(2026-09-19)＝読み手は同じ XML を「Word 後」「書き出した時(起点)」の2面で読み、**起点の指紋がタグと一致すれば変更履歴が全部**(`N with complete revision marks`)。⚠**段2の Import は Word 後の本文を丸ごと比較する**(HTML と同じ挙動)＝「Word で変えた所だけ」を出す3者合成は段3。スクリプト口＝`app.kcmExportStoryDocx(folder)`(`eKGf`・⚠Adobe 未登録)。✅実機 PASS(段1 `kcm-docx-stage1-live-2026-09-19.md`／段2 `kcm-docx-stage2-live-2026-09-19.md`＝KIDMCP)。★★**ワード方式が通れば HTML 方式は廃止**(ユーザー決定)＝提出説明を書く時点でどちらが残っているか確かめること。★段3(同日夜)＝**変更履歴が全部の docx は3者合成**(`KCMStoryMerge`＝書き出した時・Word 後・今の文書)で、**一覧に出るのは Word で変えた所だけ**(InDesign 側で後から変えた所は出ない)。Word と InDesign が同じ語を変えたら**その1件だけ断って文書の語を残し**、状態行に `N change(s) from Word, M conflict(s) kept the document's words (uid: 場所 - 理由)`。記録オフ等で履歴が不完全なら従来どおり丸ごと比較。✅実機 PASS(`kcm-docx-stage3-live-2026-09-19.md`)。
//   ㊶★**変更行の ID 欄に「Text」「Cell Text」「Note Text」**(2026-09-19 ユーザー要望「セル内の変更は Cell Text、普通は Text」＝五月蠅ければ外す)。文字の変更行の ID 欄が空だったところに、その語がどこに立つかを出す(overset は `OV Text`)。列幅は最も広い語に合わせて自動。★同日夜＝**段落丸ごとの行(+/−)は「Paragraph」「Cell Paragraph」「Note Paragraph」**(ユーザー要望「+ だけでは文字の追加か段落の追加か分からない」)。列幅は `OV Cell Paragraph` で計測。
//   ㊹★★**段落丸ごとの行(+/−)の3件を同日夜に修正**(全文＝`docs/ai-notes/kcm-whole-paragraph-marks-and-undo-2026-09-19.md`)＝(a)**印・ジャンプ・ダブルクリック選択が上の段落の改行から始まっていた**(書き戻し用の範囲 [上の改行, 段落の改行) をそのまま見せていた)⇒facade の `GetChange` が渡す時だけ改行を外す(`fBreakAt`／`KCMShownSpan`)。(b)**段落を2つ取り込んで「Undo the Restore」すると、拒否されるか「¶ba」の順で戻る**(取り込みの記録＝幅ゼロのキャレットが同位置に重なり、位置だけでは前後が決まらなかった)⇒記録を読み順で持ち、ずらす規則を「どの記録から・削除幅・挿入幅」で決める(`ReplacedSlotFor`／`ShiftReplacedChanges`)。(c)**段落スタイルが戻らない**(a:A b:B c:C を戻して戻すと全部 C)⇒取り込み時にスタイルを記録し、やり直しは**二重規則**(上の段落に次のスタイルがあればそれ・なければ記録／後続は前の段落の次のスタイルを着ていた限り連鎖)＝ユーザー決定「Next Style がある限り連鎖」。上の段落が空のときの InDesign の結合(下のスタイルが乗る)と `KCMRechainAfterRemoval` の早期 return も同時に修正。✅実機 PASS＝空段落(A→B→C の連鎖)に Word から3段落を Import→A/B/C、戻す×やり直すを4通りの順(正順・逆順・真ん中だけ・一括→混ぜ順)で全部 A/B/C に戻る。★同夜＝**スクリプトの口を2つ**(ユーザー「提案を実装しましょう」)＝(a)**`app.kcmUndoRestore(storyRow, words)`**(`eKGb`・+34・`kcmTakeInChange` の鏡＝取り込み済みの行を文字で名指しして Undo the Restore)。(b)**`app.kcmStoryRows` の子行をパネルと同じ並び(拒否→生きた＋置換済みの merge)にし、末尾に8列**(`state`=live/replaced/undone・`whole`・`place`・表示範囲 `tstart tend sstart send`・`other`)。表示範囲は facade の `GetChange` から取る＝印・ジャンプと同じ数。⚠(b) は列を足しただけでなく**子行の番号の意味が変わった**(旧＝生きた変更だけ・取り込み後にパネルとずれていた)。
//   ㊸⛔**同日夕に取りやめ**(ユーザー決定「いまの特種マークの仕組み使いにくくて」「ワードにあるコメントもいったんなくす」)＝**印も打てる文字列も「お約束」の枠も廃止**。現行＝**段落の中の表は Word には運ばず、戻すときは「今の文書」が段落の切れ目を決める**(`KCMStoryDocx::SplitAtTables`＝表は必ず単独の段落・書き手/読み手/指紋の共通の形／`RejoinTables`＝取り込みの直前に文書の形へ戻す)。Word からは「表の隣で段落を割る/つなぐ」ができない(InDesign で行う)。✅実機(KIDMCP)＝allin の 269(表だけの段落が2つ続く＝印の方式では「10 paragraph(s) became 9」で書き出しを断られていた)が書き出せ、Word で段落とセルを1か所ずつ変えて Import→**その2件だけが入り 10 段落・表2つの位置はそのまま**。試験台 `TestSplitShape`＋Word 2007 の実物5本 ALL PASS。⚠**提出説明には現行だけを書く**(印の方式は公開版に無い)。以下は経緯:
//   　旧㊸★★**Word(.docx)の「段落の中の表」を印で運ぶ・文書の先頭に「お約束」の枠**(2026-09-19 昼・段3b。ユーザー決定「印は表の前後に2つ」「文言 `⟦continues past the table⟧`／`⟦continued from before the table⟧`」「打てる文字列 `KCM:continued`」「枠からコピー」)。Word には段落の中の表が無いので、表を越えて続く段落は**段落末の印(次の表を越えて続く)と段落頭の印(前の表から続く)**＝ロック付きコンテンツコントロール(タグ `kcm-continues`／`kcm-continued`)で表し、**印の無い表は自分の段落を始める**。ユーザーは `KCM:continued` を段落の先頭か末尾に打つだけでも同じ意味(全角も可・Word の変更履歴に載るので「Word で変えた所だけ」に乗る)。**文書の先頭にお約束の枠**(タグ `kcm-legend`・ロックなし・使い方とコピー用の印。読み手は丸ごと読み飛ばす)。⚠**段落スタイル `kcm-continued` は廃止**＝`w:pStyle` は書かない・読まない(将来「InDesign の段落スタイル名」に使う枠として空けた)⇒段1〜3で書き出した docx は読めなくなる(未提出)。**表の隣で段落を割る変更は、この段では合成が「衝突」として名指しする**(取り込めるようにするのは次の計画)。実機(KIDMCP・Word 2007)＝書き出し→保存し直し→Import で 0 件／X・Y・C の3件の取り込み(段3 F と同じ)／打った文字列が宙に浮けば無視／印の直後の Enter＝衝突1件を名指し／`matrix.indd` 2ストーリー往復(⚠割注の中の縦中横は **Word 2007 の保存で `w:vert` が落ちる**＝指紋不一致→丸ごと比較に落ちて縦中横1件が行に出る。KCM の外の制限として設計書 §12 に記載)。同日午前＝**Task Start の写しで表のセルの文字が消える不具合を修正**(`<XmlStory>` の双子＝`e825754`)。
//   ㊷★**取り込みで段落を丸ごと消したとき、後続の段落を「次の段落スタイル」で付け直す**(2026-09-19 ユーザー要望「1 2 3 4 で 2 が消えたら 3 以降に次の段落スタイルが当たっていってほしい」)。消す前に「後続が前の段落の次スタイルどおりに並んでいたか」を読み、消した後に**その連なりが続く範囲だけ**前の段落から付け直す(手で別スタイルにした段落で止まる・オーバーライドは残す・同じ undo 段)。✅実機 PASS(S1 S2 S3 S4 の 2 を消して S1 S2 S3)。⚠段落を**足した**ときの後続の付け直しは未着手。
//   ㊹★★★**「Import Story Text...」は編集を文書に全部入れ、Story モードで確認する形に**(2026-09-19 夕・ユーザー発案→同日「見え方はいまのでいいですね、この方式にしましょう」で採用。設計書＝`docs/superpowers/specs/2026-09-19-kcm-import-direct-design.md`)。Import は①ファイルを読み②**Task Start を取り**(先に取ってあった Task Start は置き換わる・助言ダイアログは廃止)③編集を**実文書に全部入れ**(1つの abortable なコマンド列＝**Ctrl+Z 1回で Import 全体が戻る**・進捗バーの Cancel で無変更)④**Story モードで比較を始める**(Source＝Import 前の Task Start・Target＝編集の入った文書)。要らない変更は既存の右クリック「Restore Source Text」(1件ずつ)で戻し、「Undo the Restore」で Import 後の語に戻せる(行は残る)。★**入らなかったもの**(Word の変更が InDesign 側の編集と衝突・表構成が違う・ファイルに無いセルや脚注・段落／属性の拒否・対応するストーリーの無いファイル)は **Story Edits の先頭に赤い「!」の行**(親＝ストーリー(ストーリーの無いファイルは ID 欄「File」)・子＝種類 Word/Table/Place/Para/Attr/File と場所・理由。Δ 列の「!」は手描き cell `KCMStoryBangView`)＝Refresh Story Comparison でも残り、Stop・Clear・新しい Task Start・閉じるで消える。「!」の子行はジャンプするとストーリーへ飛びメッセージ領域に理由の全文。★**決め手＝あふれ(overset)が実物のレイアウトで即見える**(旧方式は窓なしの写しに入れていたので取り込むまで見えなかった)。⚠**旧「第4の Import モード」(2026-09-15〜)は公開版に無いので公開版から見れば存在しなかった機能**＝「削除した」と書かない(★そのコードは 2026-09-20 に撤去済み＝`kKCMModeImport`・`KCMInImportMode`/`KCMEndImportMode`・保持していた語・「Change to Imported Text」「Change Back to the Original」「Finish Import」。ActionID +72/+78 は欠番)。★**同日の決定＝割注・縦中横の変更行はどこでも戻せる**(旧「Import モードのときだけ」の例外を、モードごと消さずに規則へ格上げ)。How to Use に「ストーリーの往復」の節を英日で新設。✅実機 PASS(2026-09-19・KIDMCP＝docx の衝突1件+取り込み1件／HTML 3種(語の変更・表構造・存在しない UID)／Ctrl+Z／Restore→Undo the Restore／Refresh・Stop。全文＝`docs/ai-notes/kcm-import-direct-live-2026-09-19.md`)。
//   ㊺★★**Story Edits の親行の Story 列で、段落の中の表を ▦(U+25A6)で示す**(2026-09-19 夜・ユーザー報告「a[表]b が a b で表があると分からない」→「表の絵を置くか、表で区切るか」の比較で「置く」を採用。全文＝`docs/ai-notes/kcm-story-row-table-sign-2026-09-19.md`)。規則＝印は表のアンカーの位置に立ち(`a▦b`)、行ごとの U+0017 は出さず、表しかない段落は印を持ち越して最初の語の前に置く(`▦ 品名`・`▦ z`)。⚠記号は3段階で決めた＝`田`(日本語なので却下)→U+229E ⊞(ビルドして見せたら「あふれの＋に見える」で却下)→U+25A6。★**PDF 報告書では組版フォントに無く notdef になった**ので、報告書だけ `KCMStoryList::RowTextForTypesetting` で語 `[table]` に置き換える(`a[table]b`)。子行・diff・docx・HTML・取り込みは表の文字を持たないままで無変更。⚠**㊹ が2つある**(段落丸ごと3件と Import)＝番号は繰り上げず、次は㊻。
//   ㊻★★**変更行の ID 欄の語彙を改めた＝段落丸ごとは「Paragraph」、セル丸ごとは「Cell」**(2026-09-19 深夜・ユーザー「何もないところに表ができたら Cell が + に」「セルの中で2段目なら Paragraph でいい」。全文＝`docs/ai-notes/kcm-story-table-rows-2026-09-19.md`)。「Cell Paragraph」「Note Paragraph」は廃止(文字の変更は「Text / Cell Text / Note Text」のまま)。セル丸ごとの判定は番地でなく「そのセルの段落が全部この run の中か」(`KCMStoryChange::fWholeCell`・`app.kcmStoryRows` の whole=2)。★**空の変更行(空のセル・空の段落・唯一の語を消した Text −)は Story 列に削除と同じ縦棒を描く**(白紙だった)。⚠**「Cell」はつなぎ**＝次の回で「表の形(行・列・結合)が変わった表を Table +/−/≠ の1行に畳み、Task Start のときだけ写しから表ごと戻す」に置き換わる(ユーザー決定)。
...
DECLARE_PMID(kScriptInfoIDSpace, kKCMSaveDocXmlMethodScriptElement, kKCMPrefix + 36)	// ★★app.kcmSaveDocXml(file) -> 0 written / 1 nothing could be photographed / 2 could not create / 3 could not write / 4 bad argument (2026-09-20, the user's ask: "can the internal IDML of THAT document be made too? then they can be compared"). The ACTIVE document's own designmap, taken the same two ways Task Start takes the origin's - so a rehydrated copy can be checked as XML, element by element, instead of by the four numbers of its shape. Implementation KCMSaveActiveDocXml (KCMOriginIdml.cpp), ScriptID 'eKGg'. ⚠**+36 is the next free slot in kScriptInfoIDSpace and NOT a grave**: the highest in use was +35, while +28 and +32 ARE graves (kcmTakeInAllStories and kcmTakeInStory went with Restore All) and are not reused. ⚠+32/+36 also appear in this file under kClassIDSpace - a DIFFERENT ID space, so there is no clash
...
// ⛔**+28 IS RETIRED** (2026-09-20): app.kcmTakeInAllStories, with "Restore All Stories" itself. Never reused.
