...
//   ㉙★**Story Edits の変更行(子供の行)を右クリックすると「Copy Source Text as Plain Text」**(2026-09-12 ユーザー要望)。その変更の**ソース(旧文書)側の文字列だけ**をプレーンテキストでクリップボードへ置く＝置換なら置き換えられた元の文字列、削除なら消された文字列、**ルビの変更・削除なら元の読みそのもの**(ベース文字ではない)。挿入・ルビ追加・圏点・脚注の行では出ない(項目が1つなのでメニュー自体が出ない)。前後の文脈は含めず、InDesign の特殊文字(アンカー類)は落とす。★公開版では子供の行の右クリックは何も出なかったので、公開版から見て新機能。⚠実機未確認(2026-09-12 時点)。
