...
//   ㉘★**進捗バーを「3秒経ったら出す」に変え、Story モードにも付けた**(2026-09-05 ユーザー決定)。従来は Pixel だけ「ラスタ化するページが10以上」のとき出し、Story には無かった。両モードとも3秒で出て Cancel できる(Story のキャンセルも Pixel と同じく比較を止める)。★文言: 登録トグル / Load Check & Register のキャンセルが「Recompare cancelled - comparison stopped」「Load cancelled - comparison stopped」と、比較が止まったことまで言うようにした。内部(利用者からは見えない): 透明の一覧の自己修復＝Start/Stop のたびに一覧を「聞き直し」、本物の透明だけを残す保険。
//   ㉟★★**Before/After 報告書「Export Before/After Report」**(2026-09-13 ユーザー選択＝「Task Start が INX を保持している」ことを使う案の4番・「変更ページだけ」)。比較中にフライアウトから選ぶと、要約ページ(2文書・モード・件数・Story Edits の行・Resources の要約)＋変更ページごとに横長1ページ(左＝旧版・**比較マーク印刷**／右＝新版・素のまま＝ユーザー要望「マークはソースの方のみ」)の PDF を**保存ダイアログで選んだ場所・名前**(初期名 `<文書名>.compare-report.pdf`)に書き、ビューアで開く。左側のマークは**ページ縁の枠なし**(リングだけ・`sRingFrameOff`)、足す文字は 18pt(いずれも 2026-09-13 ユーザー要望)。追加ページは左が空・削除ページは右が空。Task Start が Source なら左は写し(再水和)。実装＝`KCMReport.cpp`(両側を一時 PDF に `kPDFExportCmdBoss` で書き出し→窓なし文書に `kSetPDFPlacePrefsCmdBoss`＋`PlaceFileInFrame` で並べ→書き出し→閉じて一時ファイル削除)。ActionID +65。⚠`SDKLayoutHelper.cpp` を model の vcxproj に足した(実物と控え)。✅実機 PASS(2026-09-13・KIDMCP `capture pdf:`)＝別名保存の対で 5 ページ(要約＋追加3・削除1)／内容ありの対で左右の絵とリング／Task Start でページ移動＋追加(左＝写し)。★facade は末尾に足し ABI 印を 2026091302 へ・KIDMCP 再ビルド済。設計書＝`docs/superpowers/specs/2026-09-13-kcm-before-after-report-design.md`。★★**同日午後に3部構成へ拡張し、名前を「Export Before/After PDF Report」に**(ユーザー指示「Story とリソースの情報も」「1P目は3行＋最下部に左 Before・右 After」「Pixel のページに文字は不要」「Report の前に PDF」「Story はパネルの情報を左右に・変化なしは薄く・変化ありは濃く・ルビも」)。①1P目＝タイトル・Before・After の3行と、足に左「Before」右「After」②Pixel の変更ページ＝左右の絵だけ③**Story の表**(2列・左 Source／右 Target・変更1件1行。文脈 40% tint・変更 100%・ルビ／圏点は本物を掛ける・脚注番号は上付き・挿入／削除の空側は「|」)④**Resources の表**(定義ごとに見出し行＋属性ごとに左旧値／右新値)。★**どのモードでも②以外は出る**＝Story の詳細と Resources の結果を報告書のために作って元に戻す(`StoryDetailLoan`／`ResourceLoan`)。Pixel の絵は Pixel モードだけ(他は1P目に `Pixel: not compared in this mode`)。表の実装＝新規 `KCMReportTable.cpp`(`ITableUtils::InsertTable`＋セルの thread へ `InsertCmd`＋`kNewSpreadCmdBoss`／`kTextLinkCmdBoss` であふれた分を次ページへ連結)。`KCMXmlPretty` を `ui/`→`source/` へ移し両プラグインでコンパイル、`KCMShortResourceValue` は共有 inline(`KCMResourceShortValue.h`)。Resources の「origin か Source か」の分岐は `KCMResourceStore::RebuildForPair` の1か所に。facade 不変。設計書＝`docs/superpowers/specs/2026-09-13-kcm-pdf-report-three-sections-design.md`。★★**同日夕方の手直し（全部ユーザー指示・実機 PASS）**＝①表を**4列「ID（Resources は Kind）｜Δ｜Before｜After」**にし、見出し行は表のヘッダー行（次ページに繰り返す）②親行の Before/After セルを結合③**Story の ID セルは同じストーリーの子行と縦に結合**（⚠結合セルはページをまたげず 80 件で 200 ページ超えになったので、**表を全ページに流した後で各行の載ったフレームを `ICellContent::GetParcelFrameUID` で聞き、同じページの分だけ結合・次ページの先頭に ID を再記入**）④ルビは本文と同じ 14pt の本物のルビで2行（圏点も）・セルに 4pt の余白（`kCellAttr*InsetBoss`）⑤1P目＝3行を 36pt・**猫の足あと 10 個（赤青交互・蛇行・`kKCMPawOutlines` からスプライン＋RGB スウォッチ＝`KCMReportPaws.cpp`）**・最下部に `Exported: 日時`。Before/After の語は Pixel の各ページの絵の上へ⑥**Resources の値に文書の単位を添える**（`KCMResourceUnits.h`＝属性名で文字サイズ／テキスト／水平／垂直／線の単位に振り分け、`IUnitOfMeasureSettings`→`IUnitOfMeasure::Format`。PDF とパネルの行・帯の3か所が同じ inline 関数）＝`8.503937007874015 (12 Q)`。全文＝`docs/ai-notes/kcm-pdf-report-three-sections-2026-09-13.md`。
//   ㊱★★**変更1件だけを Task Start(旧版)に戻す「Restore Source Text」**(2026-09-13 ユーザー選択＝「この部分だけ Task Start に戻す」。続けて「ルビも戻せると嬉しい／追加・削除も／グループルビ・モノルビをしっかり／圏点も可能なら」)。Story Edits の変更の行を右クリック→旧版側を新版のその範囲へ書き戻す＝①本文(`ReplaceCmd`/`InsertCmd`・削除は `DeleteCmd`＝2026-09-17 に公式の形へ・旧版の語は Source ストーリーから `TextIterator` で生読み・挿入は取り去る)②ルビ(`kTARubyAttrBoss`/`kTARubyStringBoss`/`kTAMojiRubyBoss` の3属性を `ApplyCmd`・旧版に無ければ 30 の kTARuby* を `ClearOverridesCmd`・**kTAMojiRubyBoss kTrue＝モノ**・strand が無ければ `kPrivateCreateStrandCmdBoss`)③圏点(`kTAKentenKindBoss` に種類・無ければ Kenten_None)。全部を `BeginCommandSequence` 1本「Restore Source Text」に包む＝Ctrl+Z 1回。守り＝行に記録した `ITextModel::GetTextChangeCount`(`KCMStoryRow::fTargetTextCount`)と今の値が違えば断る／カスタム圏点は断る／本文も変わった段落の属性(範囲 0)は断る。書いた後は `KCMStoryDiffRun::RunOne`＋通知で行が消える。実装＝`KCMStoryRestore.cpp`・facade `IKCMStoryEditsFacade::RestoreChange`(末尾に追加)・UI `KCMStoryCopy.cpp`。ActionID +66。✅実機 PASS(2026-09-13・KIDMCP)＝本文 replace/delete/insert、ルビ Group 変更・Mono 変更・Group 追加・Mono 追加(2字)・Group 削除、圏点 変更・追加・削除の計 12 件を戻して Task Start と同一(KCM 0件・`ruby` ツールの markup 一致)、Undo 1段で戻る。
//   ㉞★**ページの対応づけを「並び順」から「UID」に**(2026-09-13 ユーザー決定「UID のみ」＝閾値による自動切替は置かない)。通常ページは他方の文書の**同じ UID を持つページ**と比較され、旧版を別名保存して作った新版なら、ページの追加・削除・**移動**があっても正しい相手と組になり、新版だけのページ(追加)・旧版だけのページ(削除)は**登録なしで**赤の斜線になる(report に `added=N removed=M`)。フライアウトに check トグル **「Pair Pages by UID」**(既定 ON・ActionID +64・パネル設定に保存)を追加し、OFF で従来の並び順規則に戻せる(別々に作った2文書・IDML 経由の文書向け)。Task Start の写しはページに `KcmOriginUid` ラベルを持たせて元 UID で組む(Story と同じ道。⚠`ImportINX` はページのラベルを捨てるので取り込み後に XML の「スプレッド→ページ列」から書き戻す＝`KCMRehydrate.cpp` `LabelCopyPages`)。★同時に直した既存の穴＝Task Start で写しを離した直後にあふれキャッシュが空で作り直され、追加ページの赤「/」が出なかった(`KCMDetachArmedSource`)。規則の本体＝`KCMPagePairRule.h`(純粋関数・オフラインテスト `work/kcm-pagepair-test` 18件＋`work/kcm-origin-test` 54件)。✅実機 PASS(2026-09-13・KIDMCP)＝①別名保存の系譜 6→8 ページ(追加3・削除1・移動1)で `pages compared=7 changed=0 added=3 removed=1`・赤「/」は両文書とも正しいページ ②トグル OFF で並び順(末尾2ページが赤)→ON で戻る ③Task Start 後にページ追加＋移動で `compared=10 added=1`・移動ページ無印・追加ページに赤「/」。⚠★★★**この回に `IKCMCompareFacade` の途中へ仮想関数を足して KIDMCP を落とした**(vtable のスロットずれ＝KIDMCP の `ReleaseExternalSourceDB` が `ExportChangedPagesTSV` に着地)⇒**facade の仮想関数は末尾にだけ足し、KIDMCP も再ビルド**(ヘッダー末尾に警告を書いた)。全文＝`docs/ai-notes/kcm-page-pairing-by-uid-2026-09-13.md`。
//   ㊲★**ストーリーを持つページアイテムの上にストーリーの UID を出す＝フライアウトの check トグル「Show Story IDs」**(2026-09-13 ユーザー要望「ストーリーの UID をレイアウト画面にグローバルページアドーンメントで」「パステキストも」「ストーリーから辿ってその親に」「Print ON では印刷されてほしい」→同日午後「番号だけ・フレームの外の上に・2倍の大きさ・Before/After にも載せてよい」→「その 75%・`StoryID:` を付けて・色は黒のまま」。**最初の2時間は「Show Frame UIDs」＝フレームの中に `ID:`/`Story:` の2行**だった)。既定 OFF・ActionID +67・パネル設定 `showStoryIds`。**探し方はストーリー起点**＝`IStoryList`→各ストーリーの `IFrameList` の列→`IHierarchy` を2段上がった親(テキストフレーム `kSplineItemBoss→kMultiColumnItemBoss→kFrameItemBoss` もパス上テキスト `kTOPSplineItemBoss→kMulticolumnItemBoss→kTOPFrameItemBoss` も同じ2段)。⚠**パス上テキストの2段上は見える図形と別 boss**なので `ITOPSplineData::GetMainSplineItemUID()` で main spline へ結ぶ(1回目の実機で楕円だけ出なかった)。その item の描画で、スプレッド座標に直した箱の**上辺の左・外側**へ黒字＋白縁で `StoryID:番号`(パスにもテキストが載る frame はラベル2つ)。**全文書に出る**(セッションのグローバルアドーンメント)。画面は常時・ズーム非依存の固定 px(16.5px)、⚠オーバープリントプレビュー中は Print marks OFF なら出さない(09-07 の規則)。印刷/PDF は「Print comparison marks」ON のときだけ固定 pt(10.5pt)、**Before/After 報告書には After 側だけ載る**(`sReportExport`＋Before 側は `sRingFrameOff` で除外。ユーザー「Task Start の写しは ID が振り直されてこんがらがる」)。サムネイルには出さない。不透明なのでフラットナ申告は不要。ラベルは箱の外なので `GetPaintedAdornmentBounds` で上と右に広げる(でないと移動で残像)。実装＝リングと同じグローバルアドーンメント `KCMRingAdornment.cpp` §1.5(グローバル登録は item ごとにも呼ばれるので、その枝を使う。表＝db ごとの item UID→story UID、スプレッド描画のたびに作り直し・文書を閉じたら捨てる・クローンの書き出し開始でも捨てる＝ポインタ再利用対策)。facade 末尾に `Get/SetShowStoryIds`・ABI 印 2026091304・⚠KIDMCP 再ビルド要。
//   ㉙★**Story Edits の変更行(子供の行)を右クリックすると「Copy Source Text」**(2026-09-12 ユーザー要望。⚠同夜に一度「Copy Source Text as Plain Text」にしてから「as Plain Text」を外した＝ユーザー指示)。その変更の**ソース(旧文書)側の文字列だけ**をプレーンテキストでクリップボードへ置く＝置換なら置き換えられた元の文字列、削除なら消された文字列、**ルビの変更・削除なら元の読みそのもの**(ベース文字ではない)。挿入・ルビ追加・圏点・脚注の行では出ない(項目が1つなのでメニュー自体が出ない)。前後の文脈は含めず、InDesign の特殊文字(アンカー類)は落とす。★公開版では子供の行の右クリックは何も出なかったので、公開版から見て新機能。✅実機 PASS(2026-09-12 夜・置換/削除/ルビ変更/ルビ削除の4種と、挿入/ルビ追加/圏点でメニューが出ないこと)。⛔★★**2026-09-15 に撤去した(ユーザー指示)＝提出説明に書かない。** 公開版 1.3.0 にも 1.5.0 にも入っていないので、**公開版から見れば存在しなかった機能**＝「削除した機能」としても説明しない(Translucent Toolbox トグル・押下中 HUD と同じ扱い)。この行は履歴として残す。
//   ㉚★**ルビの種類(mono⇄group)だけの違いは変更として出さない**(2026-09-12 ユーザー決定＝2026-09-08 の「種類を出す」を撤回)。同じ読みのままモノルビ⇄グループルビを変えても Story Edits に行は出ない。あわせて、ルビ行の右上に出していた「Mono」「Group」の表示も撤去した。★公開版は種類の判定を持っていないので、公開版から見た差分は**無し**(2.0.0 の中だけの往復)＝リリースノートには書かない。
//   ㉛★★**不具合修正＝表を足した新文書で、表のセルの行(＋)をクリック／Next で辿るとクラッシュしていた**(2026-09-12 に実機で2回再現・修正後に24件を走査して再現せず)。原因＝行のジャンプ先のフレームを `IFrameList::QueryFrameContaining` で引いていたが、この API はセルの位置まで組版を起こす契約で、表を含むその組版が TEXT.RPLN 内で落ちた。修正＝ストーリー行のジャンプが元から使っているパーセル経路(`ITextParcelList::GetParcelContaining`→`IParcelList::GetParcelFrameUID`)へ置き換え、セルの位置は表のアンカー位置へ写してから引く(`KCMPrimaryIndexOf`)。★★**公開版 1.5.0 は Story モードを持たないので公開版には無い不具合**＝2.0.0 の中の修正。⇒ リリースノートには書かない。
//   ㉜★**ストーリー末尾（と段落末尾）の削除にも「｜」が描かれる**(2026-09-12 ユーザー報告「一行全部削除されたときに｜がない」＝実体は「末尾の文字を消したとき」)。削除の caret は「次の文字の前」に立てる作りで、末尾には次の文字が無く何も描かれなかった。⇒ 末尾の削除は**最後の可視文字の後ろ**に立てる(`KCMMarkRange::CaretAfter`・描画側は改行文字の前に立つ caret も前の文字の後ろへ)。★公開版は Story モードを持たないので 2.0.0 の中の修正＝リリースノートには書かない。
//   ㉝★★**不具合修正＝比較中の文書を閉じると（特に「Always Show Marks」「Print comparison marks」ON のまま）、その後の別文書の組版で落ちることがあった**(2026-09-12 に実機で再現・修正後に同手順で再現せず)。原因＝Story マークの地図が閉じた文書の DB ポインタを持ち続け、kAfterCloseDoc の掃除がそれだけ抜いていなかった。⇒ `KCMStoryMarker::ForgetClosedDocs` を掃除の先頭で毎回呼ぶ(ポインタ比較のみ)。2.0.0 の中の修正＝リリースノートには書かない。
...
//   ㊳★**パネルメニュー(フライアウト)の整理**(2026-09-14 ユーザー指示)＝①**並びをユーザーの指定順に組み直した** ②**6項目を外した** ③**3件を改名**。⚠**公開版 1.5.0 から見える変更は5つ**＝(a)**「Export Changed Pages...」(変更ページの TSV 書き出し)が無くなった**——同じ一覧は㉟の PDF 報告書が持つ (b)**「Show Scrollbar Map」のトグルが無くなり、スクロールバーの地図は常に出る**ようになった (c)「Start」「Stop」→**「Start Comparison」「Stop Comparison」**(「Refresh Comparison」と語形が揃い、「Compare Books」とも紛れない) (d)「Translucent Panel」→**「Translucent Kohaku Change Marker Panel」** (e)「Translucent Dialog」→**「Translucent Compare Books Dialog」**。★外した残り3項目(Task Start の XML 器具＝Open raw / Open copy / Save to Desktop)は**2.0.0 の中で足して 2.0.0 の中で外した**ので公開版から見れば存在しない＝リリースノートには書かない(書き出しの実装だけは㊴として残った)。⚠**外した6件の ActionID は欠番にして再利用しない**＝再利用すると、利用者が旧項目に割り当てたショートカットをそのまま新しい項目が引き継ぐ(+38・+55 と同じ扱い)。✅実機 PASS(2026-09-14 ユーザー確認「問題なしでした」)。全文＝`docs/ai-notes/kcm-menu-reorder-2026-09-14.md`。
//   ㊴★★**スクリプトに初めてメソッドを公開＝`app.kcmSaveOriginXml(<ファイルパス>)`**(2026-09-14 ユーザー決定＝㊳で外した「Save Task Start XML to Desktop」の**実装は残し、スクリプトから呼ぶ。書き出し先とファイル名は呼び手が決める**)。保持中の Task Start 起点の XML を、指定されたファイルへ Task Start が撮ったまま書き出し、**Int32 のステータス**を返す(0 書いた／1 起点が無い／2 ファイルを作れない／3 書けない／4 引数を読めない。⚠数を決めるのは `KCMOrigin.h` の1か所で、`.fr` の戻り値説明と `KCMScriptingDefs.h` はそれを言い直しているだけ)。★**KCM がこれまで公開していたのはプロパティだけ**で、「メソッドは公開しない」と3つのファイルが宣言していた——その文が指していたのは **kescmToast のような「製品を外から駆動する API」**であって、これは何も駆動しない(パネルからはもう届かない書き出し口)。ScriptID は `eKGs`・ScriptElement は `kKCMPrefix + 24`(⚠旧メソッドの墓 +1..+12 は再利用しない)・引数は Adobe 標準の `keyAEFile`。手本＝`snippetrunner` の "save snip log"。⚠**Adobe には未登録**＝次の提出で `nKGp`・`pKGx` と一緒に申請する(登録の単位は「コードと DOM 名のペア」なので、名前を変えるなら申請前に)。✅**実機 PASS**(2026-09-14)＝起点なし→1／Task Start 後→0 で **182,242 バイトの INX がそのまま書かれた**(`app.kcmOriginStatus` の bytes と完全一致)／存在しないフォルダー→4／引数なし→エンジンが「必須パラメーター『filePath』が見つかりません」(30479)。⚠**失敗した回はファイルを作っていないことも確認**。全文＝`docs/ai-notes/kcm-save-origin-xml-method-2026-09-14.md`。
...
// ★THE FIRST METHOD SINCE THE OLD ONES WERE BURIED (2026-09-14). app.kcmSaveOriginXml(file) writes
//   the held Task Start origin's XML where the caller says, and answers with a status number
//   (KCMOrigin.h decides the numbers). ⚠**It is taken from +24, NOT from the graves at +1..+12** -
//   those are retired scripting methods, and the rule at the top of this block ("new ones are taken
//   from +13 so that nothing is confused with a retired method") is about exactly this hazard: a
//   reused element ID would make an old note about kescmToast read as if it described this.
