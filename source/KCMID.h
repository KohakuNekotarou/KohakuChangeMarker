...
//   ㊲★**ストーリーを持つページアイテムに「ID:<UID>」「Story:<UID>」のラベル＝フライアウトの check トグル「Show Frame UIDs」**(2026-09-13 ユーザー要望「ストーリーの UID をレイアウト画面にグローバルページアドーンメントで」「ストーリーから辿ってその親にストーリーの ID を」「Print ON では印刷されてほしい」)。既定 OFF・ActionID +67・パネル設定 `showFrameUids`。**探し方はストーリー起点**＝`IStoryList`→各ストーリーの `IFrameList` の列→`IHierarchy` を2段上がった親(テキストフレーム `kSplineItemBoss→kMultiColumnItemBoss→kFrameItemBoss` もパス上テキスト `kTOPSplineItemBoss→kMulticolumnItemBoss→kTOPFrameItemBoss` も同じ2段)。その item の描画で、スプレッド座標に直した箱の左上へ黒字＋白縁の2行(ID の下に Story。パスにもテキストが載る frame は Story 2行)。画面は常時・ズーム非依存の固定 px(11px)、印刷/PDF は「Print comparison marks」ON のときだけ固定 pt(7pt)。サムネイルには出さない。不透明なのでフラットナ申告は不要。実装＝リングと同じグローバルアドーンメント `KCMRingAdornment.cpp` §1.5(グローバル登録は item ごとにも呼ばれるので、その枝を使う。表＝db ごとの item UID→story UID、スプレッド描画のたびに作り直し)。facade 末尾に `Get/SetShowFrameUids`・ABI 印 2026091303・⚠KIDMCP 再ビルド要。
