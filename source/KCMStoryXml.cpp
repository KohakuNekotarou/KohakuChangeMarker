...
//
//  Ported from KohakuTest's KTStoryXml::ExportToBytes -- the export itself, with the reporting
//  and the script-facing wrapper left behind. The one behavioural choice that came with it:
//  ExportInCopyInterchange rather than ExportPageitems. KT could do either (its ExportMode
//  picked between them); the InCopy form is the one that gives a story's text as <Content>
//  elements, which is what the diff reads.
//
//========================================================================================
...
bool16 KCMStoryXml::ExportStory(const UIDRef& storyRef, std::string& utf8Out)
{
	...
	{
		// takeOwnership and recycleBoss BOTH kFalse. The IXferBytes lives on this stack frame, and
		//   StreamUtil::CreateMemoryStreamWrite warns that a recycled boss may hold on to it past
		//   the point where it has gone away.
