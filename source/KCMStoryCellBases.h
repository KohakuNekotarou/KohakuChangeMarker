...
//
//  **WHY THIS EXISTS AT ALL: THE SNIPPET AND THE TEXT MODEL DISAGREE ABOUT ORDER.**
...
//
//  So counting straight down the model -- which is what every position here used to be -- puts
//  the text after a table too far along (by the cells) and the cells themselves too early (by
//  that text).
//  @warning **LengthAgrees cannot see it:** it compares TOTALS, and the totals are right either
//   way. MEASURED in InDesign on a story of three paragraphs and one 2x2 table:
...
//
//  So the body is still counted (it is the one part the two agree on), and every CELL is asked
//  of the document. The road is the one Adobe calls recommended in SnpIterTableUseDictHier:
//      ITextStoryThreadDictHier -> ITextStoryThreadDict -> GetGridID(GridAddress) -> QueryThread
//      -> ITextStoryThread::GetTextStart
//
//  **AND THE ANSWER IS CHECKED BEFORE IT IS USED, twice** -- see the two "refuse" points in the
//  .cpp. A wrong position looks exactly like a right one afterwards, so there is no symptom to
//  catch later; the story is refused instead, the same way LengthAgrees refuses one.
//
//========================================================================================
...
/** Work out where every paragraph of the model begins, as a TextIndex into the story.
	...
	@param outStarts one position per paragraph. @warning only meaningful when this returns kTrue.
	@return kFalse when the model and the document could not be matched up, in which case the
		   ...
		   **A NESTED TABLE IS NOT ONE OF THEM:** a table charges the thread it stands in, and a
		   cell is a thread, so the same reading does for both. What it costs is one thing -- the
		   tables have to be put in the order the DOCUMENT keeps them, which is not the order of
		   their anchors once one of them is inside another. See EarlierBlock.
*/
