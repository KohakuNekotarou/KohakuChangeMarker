...
#include "KCMStoryTextImport.h"    // KCMReleaseStoryText (the same hazard: a static holding strings)
...
void KCMPeekStartup::Shutdown()
{
	...
	// The folder of edited stories held for an import. **It holds PMStrings and std::strings**, so
	//   it is the same hazard the line above exists for: statics of those types destructed at
	//   unload. Dropping them touches no UI and no document.
	KCMReleaseStoryText();
