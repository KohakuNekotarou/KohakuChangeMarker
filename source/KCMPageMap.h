...
//  ステップ2(2026-07-05): 除外対応表(登録済みページを除いた順番ペアリング)。KCMCollectPageUIDs
//  の素の平坦列を直接zipしていた5箇所(KCMDoMarkChangesDoc/peek旧版取得/スプレッド再比較/
//  CMYK色サンプラ/Hide UnchangedのSource側分類)を、ここの KCMBuildPairing / KCMMapTargetToSource /
//  KCMMapSourceToTarget に置き換え済み(経緯はメモリ kescm-page-offset-idea)。
//
//========================================================================================
...
//   ⚠この形が良いことは `IKCMPageFlagsFacade.h` に自分で書いてあった——「返す形の方が良い分業
...
enum KCMPageTick
{
	kKCMPageTickNone = 0,		// 1つも付いていない
	kKCMPageTickSome,			// 一部だけ付いている(中間チェック=kMultiSelectedAction)
	kKCMPageTickAll			// 選択が全部付いている(チェック=kSelectedAction)
};

enum KCMPageRole
{
	kKCMPageRoleNone = 0,		// 比較していない、または第3の文書
	kKCMPageRoleTarget,		// 比較の Target(新)側
	kKCMPageRoleSource		// 比較の Source(旧)側
};
...
struct KCMPageToggleState
{
	...
	KCMPageTick	fTick;
	KCMPageRole	fRole;		// Register のラベル出し分け用。Check は読まない
...
//   「選択ページ」の意味を変えるときは必ずここ1箇所で。実体は KCMPageMap.cpp。
//
...
//   (ui/KCMActionComponent.cpp の UpdateActionStates)。
KCMPageToggleState KCMPageMapGetToggleState();

// ドキュメントクローズ後の生存スイープ(KCMHandleDocsClosed から呼ぶ)。閉じた文書の登録を
// 状態だけ捨てる。★閉じた db は deref しない(IDocumentList::FindDocByDataBase へのポインタ
// 比較のみ)。
void KCMPageMapSweepClosedDocs();

// (KCMPageMapClearAll(db)=1文書分の登録だけ消す口は 2026-08-17 の不具合再検査 B4 で削除した。
//  ★呼び手はリポジトリ全体でゼロだったのに、ここには「Stop(KCMDoClearMarks)から呼ぶ」と書いてあった
//  ---- 実際に Stop が呼ぶのは下の KCMPageMapClearAllDocs()。**実在しない呼び手を宣言していた**。
...
// (KCMDrawEventHandler::HandleDrawEvent の wantMarks/wantSrcMarks)に使う。
...
// pages が空ならその文書の登録を消す。実体は KCMPageMap.cpp。
void KCMPageMapReplaceRegistered(IDataBase* db, const std::vector<UID>& pages);

// 除外対応表: targetDB/sourceDB それぞれの平坦ページ列(KCMCollectPageUIDs)から登録済み
...
// ★上の KCMBuildPairing(通常ページ)とは対応の規則そのものが違うので、別関数にしてある:
...
// targetPageUID(targetDB内)に対応する sourceDB 側のページを1つ求める(内部で KCMBuildPairing を
// 使う)。targetPageUID 自身が登録済み(除外対象)か、対応表の範囲外(対応相手なし)なら kFalse で
// outSourcePageUID は不定。
// ★2026-08-16: **マスタースプレッドのページも引ける**(通常で見つからなければ KCMBuildMasterPairing
