...
//  KCMOversetApply.cpp
//
//  overset 走査の「適用」(KCMActionComponent.cpp から分離。2026-08-13 の model/UI 分割
...
//    KCMOversetScanTargetDB は static を外した——呼び手3つ(Find Overset / Refresh Overset の
//    メニュー処理と UpdateActionStates)が KCMActionComponent.cpp に残るため。
//
...
#include "KCMDrawEventHandler.h"	// sOversetOn/sOversetDB/sOversetPages/sOversetLocs/sDB(走査状態)
#include "KCMOversetScan.h"		// KCMCollectOversetLocations / KCMOversetLoc(検出そのもの)
#include "KCMID.h"				// kKCMOversetRescannedMessage
#include "KCMModelNotify.h"		// KCMNotifyDocs(model は UI を呼ばない ---- 知らせるだけ)
// ★2026-08-13(Task 10): UI 側ヘッダー3本(KCMThumbnailRefresh / KCMScrollMap / KCMChangeNav)の
//   include を落とした。あふれ走査の結果をどう見せるかは、通知を受けた UI が決める。

/* KCMApplyOversetForDoc(KCMOversetApply.h で宣言) — db を走査して overset を反映する共有処理。
...
void KCMApplyOversetForDoc(IDataBase* db)
{
	...
	//   responder が漏れて閉じた文書のポインタが来た場合に、下の走査(KCMCollectOversetLocations)で
	//   解放済み IDataBase を deref しないよう、ここで一度だけ生存確認する(FindDocByDataBase への
	//   ポインタ比較のみ=deref しない。KCM 全体の共通規約)。死んでいたら何もせず戻る。
	...
	//   文書は UID まで同じ」(KCMChangeNav.h)なので、前の文書の基準点を持ち越すと UID の偶然一致で
	...
	std::set<UID> affectedA(KCMDrawEventHandler::sOversetPages);	// 新集合(swap 後の状態)
...
IDataBase* KCMOversetScanTargetDB()
{
	if (KCMDrawEventHandler::sDB != nil)
		return KCMDrawEventHandler::sDB;	// = KCMNavDoc() の armed 分岐と同じ
	return KCMActiveDocDB();
}

// KCMOversetApply.cpp 終わり。
