...
// IKESCMCompareFacade::ApplyOversetForDoc, so grepping for this name alone finds the other two
// and the facade forwarder -- never the files that actually ask for a scan:
//   1. Find Overset, switching ON      -- ui/KESCMActionComponent.cpp, through the facade
//   2. Refresh Overset                 -- ui/KESCMActionComponent.cpp, through the facade.
//                                         Needs Find Overset ON, NOT an armed comparison.
//   3. Start, when Find Overset is ON  -- KESCMComparisonRun.cpp:163. Re-binds the scan to the
//                                         comparison Target so marks and overset share one
//                                         Prev/Next cycle.
//   4. Stop, when Find Overset is ON   -- KESCMComparisonRun.cpp:116. The ONE caller that hands
...
//  IKESCMCompareFacade::GetOversetScanTargetDB(), so this spelling appears in exactly one
//  place besides the definition -- the facade forwarder. Grep BOTH names, or the callers look
//  like none at all (the same trap B5 hit with KESCMBaseScreenOpacity on 2026-08-17).
