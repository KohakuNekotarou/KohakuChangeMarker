...
//    ⚠KCMPageNumberMarker とは**引数が2つ違う**: あちらは bIncludeSectionName=kFalse(実ノンブルに
...
//    ユーザー指定 2026-08-18)。判定は KCMIsPageOnHiddenSpread、添えるのは PageDisplay の中だけ。
...
//    マスターどうしは KCMBuildMasterPairing が「名前で」対応付け、相手のいないマスターは組まずに
...
// Project includes:
#include "KCMCore.h"				// KCMSetStatus / KCMCollectPageUIDs / KCMCollectMasterPageUIDs
									// / KCMIsPageOnHiddenSpread(隠れたスプレッドのページを一覧から外す。2026-08-18)
// (★KCMUIShared.h は 2026-08-13 Task 10 で外した＝ステータス行への出力は Task 9 で戻り値へ変わり、
//  このファイルから UI を呼ぶ経路は1つも残っていない。保存先パスは呼び手のフライアウトが表示する)
#include "KCMDrawEventHandler.h"	// sEntries / sDB / sSrcDB / sOverflowT / sOverflowS(★どれも「比較した時点」の
									// 控え＝画面・サムネイル・地図が見ているものと同じ。2026-08-18 の B10)
#include "KCMPageMap.h"			// KCMPageMapCollectRegistered
									// (★KCMBuildPairing の呼びは B10 で無くなった＝あふれ集合は上の控えから読む)
#include "KCMChangedPagesTSV.h"

namespace
{

// 出力は2列(Page / Type)。ラベルは全て英語で統一(ユーザー指定 2026-07-25。KCM の他 UI と同じ)。
...
// 区切り文字は AppendW(UTF32TextChar) で明示的に入れる(KCM の他所と同じ流儀。char リテラルの
...
//   どのセクションの 12 かが分かる方が良い。⚠KCMPageNumberMarker は kFalse(用途が違う。下記)。
...
//     変更ページ 2,3 が "1,2" と1つずつずれた一覧になっていた(実測)。KCM の標準的な使い方
//     ---- 変更なしを隠す → 一覧を書き出す ---- がそのまま当たる。
//   ⚠**KCMPageNumberMarker.cpp は kFalse のままで正しい**: あちらは「誌面に実際に刷られる数字」の
...
PMString MasterPageDisplay(IDataBase* db, UID pageUID)
{
	...
	// スプレッド」を返す契約でページ限定ではない(KCMPeek / KCMChangeNav と同じ聞き方)。
...
// KCMPanelObserver::KCMDocNameFromDB と同じ解決経路(セッション→app→docList→FindDocByDataBase)。
...
bool16 CollectRows(IDataBase* targetDB, IDataBase* sourceDB, std::vector<KCMChangeRow>& rows)
{
	...
	//   実装はここで KCMBuildPairing を呼び直し、**今の文書構成から**あふれを計算していた。
	//   キャッシュのほうは比較した時点で固定される(KCMDrawEventHandler.h の "生のページ挿入/削除
	...
	//   ★副産物: KCMBuildPairing の戻り値 tPairs/sPairs は**一度も使っていなかった**(旧ページ列を
	//     廃した 2026-07-25 以降)。あふれ集合のためだけに両文書の全ページ走査を回していたのが丸ごと消える。
	//   ⚠EnsureOverflowCache は控えた (sDB,sSrcDB) が現在と一致していれば何もしない。呼び手
	//     (KCMExportChangedPagesTSVRun)が targetDB/sourceDB をその2つから取っているので必ず一致する。
	...
	// ★マスタースプレッドは IMasterSpreadList の別管理で、上の KCMCollectPageUIDs には一度も現れない
	//   (2026-08-16 に中身が IPageList へ移ったが、**マスターを含まないのは契約**＝`IPageList.h:81`
	//    "does not include master pages"。根拠が ISpreadList からヘッダーの明文へ移っただけ)。並び順は Prev/Next(KCMBuildStops)と peek が使うのと同じ
	//   KCMCollectMasterPageUIDs に借りる(マスターの列挙順を決める場所を2つにしない)。
	//   位置は「Target の通常ページを全部出した後・Source の削除の前」= Prev/Next の巡回順と同じ並び。
	// ★Inserted/Deleted は扱わない(冒頭の注記): 相手のいないマスターは KCMBuildMasterPairing が
...
static void KCMExportChangedPagesTSVRun()
{
	...
	// 保存先(sdksamples/common の共通チューザ)。タイトル/初期名は KCM の他 UI と同じく英語固定。
	...
	//   KCMHideUnchanged.cpp の CAlert と同じ:
	...
	//     （Task 9B の KCMGetPanelBookFile とまったく同じ形）。チューザは BG では開けない。
...
//========================================================================================
// KCMExportChangedPagesTSV(KCMChangedPagesTSV.h で宣言)
...
//========================================================================================
// KCMClearExportMessage(KCMChangedPagesTSV.h で宣言)
