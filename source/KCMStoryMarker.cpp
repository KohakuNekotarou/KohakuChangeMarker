...
#include "KCMDrawEventHandler.h"	// SelectedMarkColor (the panel's red/cyan) and KCMSetOutputColor
									...
									// frames. ⚠KCMConstants.h was included here for kKCMRingR/G/B
									// until 2026-08-24; asking SelectedMarkColor instead left it unused.
#include "KCMID.h"				// ★2026-08-23: moved here from the UI plug-in's KCMUIID.h
...
#include "KCMThreadSafety.h"		// ★★KCMIsSameDoc (the background's cloned DB) and the lock
...
//   2026-08-23 and is the reason every access below takes KCMMarkStateMutex(). This file used to
...
//   registry resolves in every execution context including background threads** (KCM.fr spells
...
//     already uses (KCMDrawEventHandler.cpp:2039-2040).
...
//   them together under the per-document rule (KCMStoryMarkDocs.h), and it is the only one the
...
KCMStoryMarkDocs gMarkDocs;				// database -> story UID -> its ranges, each list merged.
											...
											// KCMIsSameDoc() instead. Walking is free: there are
...
/* KCMStoryMarkerRangesFor
   ...
   the worst kind: correct on screen, blank in the exported file. KCMIsSameDoc() asks the file
   instead, and it is the same call the comparison marks were converted to in 2026-08-15
   (KCMThreadSafety.h records the measurement).
   ...
   ⚠THE CALLER MUST HOLD KCMMarkStateMutex: the returned pointer points into gMarkDocs.
...
bool16 KCMStoryMarkerFindRunRanges(const IWaxRun* waxRun, bool16 forPrint, KCMMarkRangeList& outRanges)
{
	...
	//   the main thread rewrites (KCMStoryMarkerSetDocs), so it is all inside.
...
/* KCMStoryMarkerSetDocs
   ...
   ⚠THE CALLER HOLDS KCMMarkStateMutex. There is exactly one caller (KCMStoryMarkerInstall) and
...
void KCMStoryMarkerInstall()
{
	...
	//   "hold the lock only for the memory you are changing" is the one KCMThreadSafety.h states).
...
/** Lays a coloured wash under the marked characters. On screen always; on paper and in an exported
	PDF when the document's toggle says so (KCMStoryMarkPrintAllowedFor). ★"Screen only" until
											   ...
											   std::vector<PMRect>& outBoxes)
{
	...
	for (KCMMarkRangeList::const_iterator r = runRanges.begin(); r != runRanges.end(); ++r)
	{
		...
		if (r->fCaret)
		{
			...
			//     (KCMStoryMarkRanges.h), but what is DRAWN is a bar standing where the caret would
									 ...
									 const IWaxRenderData* renderData, const IWaxGlyphs* waxGlyphs)
{
	...
	//   is read once by KCMStoryMarkerInstall (MarkOpacityNow) and every caller ends there. The
	...
	//   installed (KCMStoryMarkRanges.h). ⚠With Difference an overlap punched a hole (two
									...
									TextIndex from, TextIndex to)
{
	...
	//   press say the same thing about the same deletion (KCMStoryMarkBuild).
...
void KCMStoryMarker::Shutdown()
{
	...
	//   ★Same door, and the same reason, as KBS's marker shutdown.
	{
		...
		//   exists to prevent (KCMThreadSafety.h).
		KCMMarkStateLock lock(KCMMarkStateMutex());
		gShutdown = kTrue;
		gHasMark = kFalse;
		gMarkDocs.clear();
