...
//
//  自動ページ番号(ノンブル)マーカーの検出(KCMPageNumberMarker.h で宣言)。
//
//  検出手順(pageRef 1ページぶん):
...
//     KCMTextRangeHasPageNumberMarker のコメント)。
...
//     (2026-07-07。KCMRealNumberInkInSpread)。フレームから少しはみ出すグリフ(大きなノンブルの
	...
	IDataBase* db, UID pageUID)
{
	...
	if (pageList == nil)
		return result;
	PMString numStr;
	// KCMDrawEventHandler の「現在番号(cur)」と同じ呼び方(IPageList.h:141-146):
	//   第3 bIncludeSectionName=kFalse … ★番号のみ(2026-08-06 の監査で kTrue から修正。バッジ側
	//     KCMDrawEventHandler.cpp の「現在番号(cur)」も元から kFalse)。kTrue が返すのは "A:12" 形式で、
	...
	//       実ノンブルと同じ数え方でなければならない。⚠同日に TSV(KCMChangedPagesTSV.cpp)・
	//       Prev/Next(KCMChangeNav.cpp)・Story Edits(KCMStoryJump.cpp)の3つは kTrue へ変えたが、
...
//========================================================================================
// KCMAppendPageNumberMarkerRects(KCMPageNumberMarker.h で宣言)
...
void KCMAppendPageNumberMarkerRects(const UIDRef& pageRef, std::vector<PMRect>& outRects)
{
	...
	if (masterPage != nil && masterPage->IsValid())
	{
		...
		for (int32 i = 0; i < nMaster; ++i)
		{
			...
			// プレースホルダ文字で組まれるが、KCMRealNumberInkInSpread は「ベースライン/フォント/変換」
...
//========================================================================================
// 除外矩形のキャッシュ(KCMPageNumberMarker.h で宣言。2026-08-06 の監査 E-3)
...
//
// ⚠★★**なぜここはロック(KCMMarkStateLock)を取らないのか**(2026-08-17・不具合再検査 B3 の2周目で
...
//       ①KCMDrawPageNumberMarkerFill(描画) …… 呼び出し側が `fillExcluded = !printing && …` で
...
//       ③★**ブック比較(KCMBookCompare.cpp の KCMGetPageNumberMarkerRects 呼び2本)** …… 章の対を
//         1組ずつ開いて比べる経路。これも main。
//         ⚠2026-08-19(不具合再検査 B-U5 3周目)に行番号(:284-285)から関数名へ。**+11 ずれていた**
//           (実体は 273-274)。同じ +11 が KCMDrawEventHandler.h の KCMRasterizingGuard の参照にも
...
//       ④KCMSetIgnorePageNumberMarker / KCMHandleDocsClosed / Shutdown(捨てる側) …… いずれも main
//   ⇒ ★**DropAllOrig と同じ形の「読み手が main だけだから守らなくてよい」**(あちらは
//     KCMDrawEventHandler.h に理由が書いてある。ここには書いていなかったので足した)。
//   ★★**この前提が崩れる変更**＝「緑ベタ塗りを印刷/PDF にも出す」。そのときは
//     KCMGetPageNumberMarkerRects と KCMInvalidatePageNumberMarkerRects の両方に
//     KCMMarkStateLock を入れること(捨てる側だけ守るのは無意味＝B3 §5 で踏んだ形)。
//========================================================================================
