...
// Project includes:
#include "KCMBoundaryID.h"	// IID_IKCMSTORYEDITSFACADE。★2026-08-17 に KCMID.h から絞った
								// (理由は IKCMCompareFacade.h の同じ位置)
#include "KCMStoryStamp.h"	// KCMStoryChangeKind を借りるため。⚠2026-08-17 訂正＝旧「a type only」は
...
public:
	...
	/** One row of the Story Edits list, in the five fields the panel actually reads.
		...
		★A COPY, NOT A POINTER INTO THE LIST. The model hands out const KCMStoryRow* today,
	...
	*/
	struct Row
	{
		...
		/** Whether the two versions' TEXT was actually diffed for this row (2026-08-21).
			...
			be no row (KCMStoryStamp.h). It means the WORDS come out the same. */
	...
	struct Change
	{
		...
		//   reading each. See KCMSnippetText.h for how the two are told apart in the snippet.
		PMString	fRuby;
		PMString	fOtherRuby;

		// WHICH attribute this is (2026-08-22): 0 = none, 1 = ruby. (2 was kenten, no longer
		// reported - KCMStoryStamp.h.)
	...
	/** Compare row nth's story again against the older document, and replace its differences with
		...
		not "does this row still belong here" (user's call, 2026-08-21). See KCMStoryDiffRun.h
	...
	/** The first frame this story is placed in. kInvalidUID when the document has no such story
		...
		(KCMStoryStamp.h:46-51). For two documents that are not versions of each other nothing
