...
//
//  See KESCMStoryMarkerExpiry.h for why this is an idle task and not a callback timer.
//
...
static const uint32 kKESCMStoryMarkerLifetimeMs = 1000;
...
class KESCMStoryMarkerExpiryTask : public CIdleTask
{
public:
	KESCMStoryMarkerExpiryTask(IPMUnknown* boss) : CIdleTask(boss) {}

	virtual uint32 RunTask(uint32 flags, IdleTimer* idleTimer);
	virtual const char* TaskName() { return "KESCMStoryMarkerExpiryTask"; }
};

CREATE_PMINTERFACE(KESCMStoryMarkerExpiryTask, kKESCMStoryMarkerExpiryImpl)

uint32 KESCMStoryMarkerExpiryTask::RunTask(uint32 /*flags*/, IdleTimer* /*idleTimer*/)
{
...
void KESCMStoryMarkerExpiry::Start()
{
	if (sShutdown)
		return;

	if (sTask == nil)
		sTask = ::CreateObject2<IIdleTask>(kKESCMStoryMarkerExpiryBoss);
	if (sTask == nil)
	...
	sTask->UninstallTask();
	sTask->InstallTask(kKESCMStoryMarkerLifetimeMs);
}

void KESCMStoryMarkerExpiry::Stop()
{
	if (sTask != nil)
		sTask->UninstallTask();
}

void KESCMStoryMarkerExpiry::Shutdown()
{
...
// End, KESCMStoryMarkerExpiry.cpp.
