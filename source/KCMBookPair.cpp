...
//
//  @warning **the front-tab walk is not in this file**: it lives in
//   ui/KCMBookPanelLookup.cpp, because it needs PaletteRefUtils, IBookUIUtils and IPanelMgr,
//   and a model plug-in may reach none of them; the resolver below is handed the answer. The
//   walk itself is ported from KBS (KBSBookScope::GetPanelBookFile), measured on this machine
//   and in use since. What is NOT ported is KBS's fall back to the active book when no front
//   tab is found -- the header says why.
//
//========================================================================================
...
/** Collect a book's chapters IN BOOK ORDER - the order the pairing pairs them in.
    ...
    apart.
	    ...
	    **EVERY POSITION IN THE BOOK PRODUCES EXACTLY ONE ENTRY, even one that cannot be read.**
	    ...
	    silent `continue` here would break both at once:
	    ...
	    It also contradicts the rule the pairing states further down this file and keeps: a chapter
	    that cannot be compared gets a ROW WITH A REASON, because a missing row reads as "nothing
	    to say about this chapter", which is the one meaning it must never carry. */
...
/** The book's display name: IBook::GetBookTitleName().
    That INCLUDES the .indb extension -- measured: an open book called new.indb reports
    "new.indb", not "new".

    **File-local.** Its one caller is the fallback in KCMBookDisplayPath, right below; nothing
    outside this file has asked for a book's bare name since the panel and the dialog went over
    to full paths. */
PMString BookDisplayName(IBook* book)
{
	...
	PMString name = book->GetBookTitleName();	// includes the .indb extension (measured)
...
/** The word for one BookContentStatus::State.
    ...
    **Empty for kDocNormal.** "The book has nothing to add" is not a reason, and appending
    "(normal)" to a failure would say less than saying nothing. The caller only appends a
    non-empty word, so a chapter the book considers fine fails with the plain sentence it
    always had. */
...
bool16 KCMResolveBookPair(const IDFile& panelBookFile, IBook*& outTarget, IBook*& outSource)
{
	...
	// The front tab is OBSERVED BY THE UI and handed in (the walk lives in
	// ui/KCMBookPanelLookup.cpp because it needs WidgetBin, IBookUIUtils and IPanelMgr, none of
	// which a model plug-in may reach). The rule is this file's: that file's book is the Target,
	// the first OTHER open book is the Source.
	// @warning a caller that could not identify a front tab must not call this at all -- it must
	//   NOT hand in a blank file and it must NOT fall back to the active book (see
	//   KCMBookPanelLookup.h).
	// **"IS IT LISTED" IS NOT THE SAME QUESTION AS "IS IT OPEN".** A book that is closing is still
	// on IBookManager's list, still reports IsOpen and still has a live database at the moment its
	// close is broadcast -- measured on the release build as "books=1, ours listed, IsOpen=1,
	// db=1". The lookup alone therefore hands back books that nothing may be run against;
	// IBook::IsOpen is the flag the close clears, so it is asked as well. KBS keeps the same two
	// tests together and says not to separate them when tidying
	// (KBSBookScope::FindOpenBookByPath).
	...
	// comparison's KCMFirstOtherDoc (KCMComparisonRun.cpp, model side), including its known
	// limitation: with three or more books open this picks one of them arbitrarily. That is
	// survivable only because both names are always shown on screen -- see the caller.
...
PMString KCMBookDisplayPath(IBook* book)
{
	...
	// IBook::GetBookFileSpec names the book's own file -- no walk of the Book panel needed, and no
	// second source of truth: this is the same book object the comparison is about to be handed.
...
void KCMBuildChapterPairing(IBook* target, IBook* source, std::vector<KCMChapterResult>& out)
{
	...
	for (size_t i = 0; i < pairCount; ++i)
	{
		...
		//
		// @warning **only where a file would actually have been opened** -- that is, for a PAIR.
		//   Added and Deleted are finished answers reached without reading anything, so overwriting
		//   one with "failed" would throw away a correct answer to say we could not do something we
		//   were never going to do. The header states the principle: "having no counterpart IS the
		//   answer; nothing about it needs opening or comparing."
		if (result.fState == kKCMChapterUnknown &&
		    (!targetChapters[i].fHasFile || !sourceChapters[i].fHasFile))
		{
			result.fState = kKCMChapterFailed;
			// "in the book" is what separates this from the other no-file reason: OpenChapter's says
			// the CHAPTER named no file, this one says the BOOK does not. The verdict itself is not
			// repeated here -- see KCMBookResult.h's fWhy.
...
PMString KCMChapterStatusText(IBook* book, int32 chapterIndex)
{
	...
	// The index is the pairing's, and the pairing was built from this same list -- but it was
	// built BEFORE the chapters were opened, and this runs after. A book edited in between would
	// make the index name a different chapter, so it is bounds-checked and no word is given rather
	// than a wrong one. (Nothing in this plug-in edits a book; the caller is a failure path, which
	// is exactly where an assumption is least worth making.)
	...
	//
	// Exists() is asked even so. Utils.h's own class comment gives the bare one-line call as the
	// ordinary form (what KBS uses in KBSBookScope::OpenChapterDoc) and offers Exists() for
	// "the plug-in that supplies the interface has been removed" -- which APPFRAMEWORK is not. The
	// difference is which path this sits on: the function only runs once a chapter has ALREADY
	// failed to open, the one path a working test run never takes. A nil here would turn a
	// reportable failure into a crash, found by a user rather than by us, and the test that
	// prevents it is one comparison.
