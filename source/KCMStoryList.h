...
/** One difference inside one story: where it is, what sort it is, and what it reads.
	...
	story changed, which is all the change counters in KCMStoryStamp can answer. These say
	...
	here, while the diff still has them (KCMTextDiff::Change carries the a-side range as well
...
struct KCMStoryChange
{
	...
	/** What sort of thing changed.
		...
		⚠WHAT IT DOES CHANGE is how fText / fOtherText are to be read. A text change shows whichever
		  ...
		  from" has to test this field before fKind (KCMStoryJump's message-area label did not,
	...
	/** ---- ruby, and ONLY meaningful when fWhat is kAttr (2026-08-22) ----
		...
		⚠MONO AND GROUP ARE BOTH IN HERE AND THE DIFFERENCE IS NOT IN THE STRING. 琥珀 read as
		  ...
		  what the diff compared (KCMSnippetText.h), and fTargetStart/fTargetEnd is the span this
		  change is about. */
	PMString	fRuby;
	PMString	fOtherRuby;

	/** WHICH attribute this is, when fWhat is kAttr (2026-08-22). kKCMStoryAttrNone for a text
		change.
...
struct KCMStoryRow
{
	...
	/** Whether the two versions' TEXT was actually put side by side for this row.
		...
		(KCMStoryStamp.h); what this says is that the WORDS come out the same. */
...
/** The first frame a story is placed in - where a jump to that story should go.
	...
	feature does: saving under a new name carries the UIDs across (KCMStoryStamp.h, "WHY TWO
...
/** Where a story BEGINS on the page, as a pasteboard point - what a jump to it should centre.
	...
	corner (the preamble to KCMLastPlacedOutport in KCMOversetScan.cpp, at the line that says the
...
/** Which frame holds ONE character of a story - the frame a jump to a CHANGE has to bring into
	...
	the view has to be showing the right spread before a point means anything (KCMChangeNav.cpp's
	KCMEnsureSpreadInView says so in as many words). In a story threaded across several spreads,
...
namespace KCMStoryList
{
	/** Replace the list with one row per entry in diffs, each read out of the document that holds it.
		...
		KCMStoryStamp.h ("READING COUNTERS COMPOSES NOTHING"): looking at what changed costs no
		recomposition.
	...
	*/
	...
	/** Drop the rows whose story differs only in HOW IT IS SET - a font, a colour, a style, a
		...
		moment before the diff was about to find the ruby. ⇒ KCMRebuildStoryEdits owns the
		...
		standing (2026-08-21, and see KCMStoryDiffRun::RunOne): refreshing answers "what differs
	...
	*/
	...
	/** Read row nth's own fields out of the target document again: the words it shows, the frame a
		...
		from the target's text, and those only ever move forward (KCMStoryStamp.h) - re-reading
