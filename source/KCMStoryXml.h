...
//
//  **ONLY ExportStory CAME ACROSS FROM KohakuTest.** KTStoryXml also has Capture/GetLast, which
//  exist to expose the export to a script (app.ktStoryXml) so the experiment could be driven
//  from outside. KCM calls this from C++ and has no such need, and a function nobody calls is a
//  promise nobody keeps -- so they were left behind rather than carried over unused.
//
//  @warning **THE XML BACKING STORE MUST NOT BE PASSED HERE.** Asking the snippet API to export
//   it crashed InDesign outright the first time it was tried. ISnippetExport's own
//   ExportInCopyInterchange says it "returns kFailure" for that case, which is gentler than what
//   actually happens. Callers walk stories with IStoryList::GetNthUserAccessibleStoryUID, which
//   does not include it.
//
//========================================================================================

#ifndef __KCMStoryXml_h__
#define __KCMStoryXml_h__
...
namespace KCMStoryXml
{
	/** Exports one story to XML, in memory. Nothing is written to disk.
		...
		@param storyRef IN the story to export. @warning NOT the XML backing store (see the file comment).
