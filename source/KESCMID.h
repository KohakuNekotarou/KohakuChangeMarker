...
//#define kKESCMVersion		"1.5.0"						// Version of this plug-in。About ボックス本文・.rc の FileVersion・PluginVersion リソースの3か所に出る。1.0.1 → 1.1.0(2026-07-25) → 1.1.1(2026-07-26) → 1.2.0(2026-07-30) → 1.2.1(2026-08-06) → 1.3.0(2026-08-07) → 1.3.1(2026-08-07) → 1.4.0(2026-08-09) → 1.5.0(2026-08-15) → **1.6.0(2026-08-19)**。
														...
														//
														// ■■【1.6.0 = 次に提出する分】★公開版 1.3.0 から見た増分。**提出説明はここだけを使う**。(版数は 1.3.1→1.4.0→1.5.0→1.6.0 と繰り上げ。①〜⑦は 1.4.0 のとき、⑧⑨は 1.5.0 のときと同一で、2026-08-19 に⑪が加わった。⚠**⑩は「設定が初期化される」という注意書きで、機能の増分ではない**＝番号を振るときに数え間違えやすい)
...
//   ⑦UI の細部2件(2026-08-12 ユーザー指定。どちらも公開版 1.3.0 から見て目に見えるが、**提出説明に書くほどではない**)。①**Story Edits の分割バーをドラッグで動かせなくした** ＝ 上ペインは固定座標のコントロールの塊で正しい高さが1つしか無く(Top snap がその高さ)、下げられる方向だけが残っていて、下げると下に何も無い帯ができた。セクションの高さはパネルの縁のドラッグで決まる。②フライアウトの「Compare Books」を**Start の直下**へ(9.54→9.05)＝比較を始める項目を1つの群にまとめた。あわせて Hide Unchanged Spreads と**同じ位置番号 9.54 で重複していた**のも解消。
														...
														//     ⚠**同時に PDF 書き出し用の描き方も作り直した**(2026-08-16 に完成)＝**印刷と PDF 書き出しを同じ処理に統一**し(アルファサーバ＋透明グループ)、**マークを CMYK で塗る**ようにした ⇒ **[PDF/X-1a] で書き出しても「非 CMYK カラー」の警告が出ない**。**印刷経路は従来どおり**。★★**提出説明に必ず書く要点＝書き出しの互換性は「Acrobat 5(PDF 1.4)」以上にすること**(PDF 1.3 は透明そのものを表現できず、透明を1つも含まないページで枠が全面ベタになる。How to Use にも明記済み)。⚠★**旧記述「PDF 書き出しのポートは透明を一切通さないので、リングをベクターで塗り不透明度を色に溶かす」は 2026-08-16 に撤回した**(欠けていたのは公式が要求する追加初期化＝透明グループだけだった)＝**2026-08-17 の不具合再検査 B3 でここへ反映**。
