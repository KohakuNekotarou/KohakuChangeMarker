...
//     ⑨-c ★★**ストーリーの4カウンターは IDML に書き出されない**(2026-08-18)＝スクリプトからは読めるまま、**利用者が書き出す IDML には一切現れない**。プロパティは既定では IDML の属性になる(実測で全 `<Story>` に載っていた)ので、INX 専用のスクリプトリソースで明示的に除外した。**提出説明に書く必要は無い**(利用者から見て何も変わらない)が、**IDML を扱う利用者にとっては「自作プラグインがファイルを汚さない」保証**なので、訊かれたら答えられるようにここに残す。
														...
														//   ⑭★★**マークを「押している間」だけでなく「常に」出せるようにした**(2026-08-22)。フライアウトに **「Show Marks on Target」** を新設(「Show Marks on Source」の直上)。ON の間は、KESCM ツールで左ボタンを押さなくても Target 文書のマークが出たままになる。★**Pixel では比較の枠、Story では変更した文字に敷く色地**＝**同じトグルが両モードで意味を持つ**(描く機構は別々だが、ユーザーから見た意味は1つ)。⚠**画面だけ**＝Target 文書が印刷/PDF に何を出すかは従来どおり「Print comparison marks」が単独で決める(Source 版が印刷にも出るのとは非対称・意図的)。
														...
														//     ★★**「Show Marks on Source」が Story モードでも効くようになった**＝Target と Source を**両方 ON にすると新旧の窓に同時に**マークが出る。
														//     ⚠★★**公開版からの挙動変更が1つ**＝**「Show Marks on Source」が Start のたびに ON へ戻らなくなった**(2026-08-22 ユーザー判断)。**両トグルとも既定 OFF で、Start は触らない。** 理由＝この設定は Save Panel Settings に保存され**起動時に自動で復元される**ので、Start が上書きすると**保存した選択が比較のたびに消える**。⇒ リリースノートに書く価値がある(公開版 1.3.0 を使っている人には「Start したら Source にも枠が出ていた」が既定だったため)。
														...
														//     ⚠★★**公開版からの挙動変更**＝1.3.0 の「Hold to Hide Marks」は「枠を常時表示し、押している間だけ隠す」トグルだった。**前半は今の「Show Marks on Target」と同じ**なので、**あのトグルを ON にして使っていた人は「Show Marks on Target」を ON にすれば同じ挙動になる**(押している間だけ隠れるところまで含めて)。⇒ **機能は1つも失われていない**が、**メニュー項目が1つ消えるのでリリースノートに書く**。
														...
														//   ⑲★**ページパネルの右クリックの2項目を、比較モードに合わせた**(2026-08-24 ユーザー指摘)。(a)**「KCM: Refresh Page Comparison」を Story Changes モードでは出さない**＝この項目が作り直すのは画素比較の結果で、Story モードはページを1枚もラスタ化しないため、押しても時間がかかるだけで画面が変わらなかった。Story モードの更新は Story Edits の行の右クリック「Refresh Story Comparison」が持つ(⇒ どちらのモードにも更新の口がちょうど1つずつ在る)。(b)**「KCM: Check」を Story Changes モードでは全ページに付けられるようにした**＝Pixel モードでは「比較枠や斜線の付いたページだけ」に限っており、Story モードは枠を作らないので**項目そのものがメニューから消えていた**(無効な項目はコンテキストメニューに出ないため)。⇒ Story モードでは確認の済んだページを自由にチェックできる。★Pixel モードの挙動は従来どおり(枠のあるページだけ／枠が消えたらチェックも外れる)。
...
DECLARE_PMID(kClassIDSpace, kKESCMStoryMarkerBoss, kKESCMPrefix + 32)	// IK2ServiceProvider(API 既製の kGlobalTextAdornmentServiceImpl)+IID_IGLOBALTEXTADORNMENT: Story モードで変わった文字の下に色地を敷くグローバルテキストアドーンメント(KESCMStoryMarker.cpp。⚠2026-08-24 までは Difference 合成による反転だった＝紙に出せず作り直した)。★★★**2026-08-23 に UI 側(kKCMUIPrefix + 30)から移設**＝**UI の File>Export>PDF は BG で走り kUIPlugIn には描画が1度も配られない**ので、紙・PDF に出すには model 側に居るしかなかった。⚠**ページアイテムアドーンメント(+29)と違って実行コンテキストごとの手動登録は要らない**＝サービスなのでレジストリが BG も含めて解決する(KESCM.fr の kKESCMRingAdornmentStartupBoss の説明が対比で書いている)
...
DECLARE_PMID(kImplementationIDSpace, kKESCMStoryMarkerAdornmentImpl, kKESCMPrefix + 51)	// IGlobalTextAdornment 実装(KESCMStoryMarker.cpp)。変わった文字の下に色地を敷く(★2026-08-24 に反転から変更)。★2026-08-23 に UI 側(kKCMUIPrefix + 41)から移設
