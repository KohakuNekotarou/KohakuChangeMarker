...
//     ★★2026-08-11 の仕上げ2件(どちらも公開版 1.3.0 から見て目に見える): ①**一覧の本文が途中で切れなくなった** = モデル側にあった30文字上限を撤去し、省略はセルの kEllipsizeMiddle に一任した(パネルを広げた分だけ本文が伸びる)。旧実装では "STORY A bottom right of page 1" でちょうど切れ、続く ", EDITED." が出ていなかった。②**ステータス欄が5px高くなった**(下端 145→150) = 日本語UIのパレットフォントは1行 17.9px(実測)で4行に 74px 要るのに枠が 69px しかなく、**4行目が約5px 切れていた**。どちらも「表示が欠けていたのを直した」ので、提出説明では機能追加ではなく修正として書く。
														//     ★★★2026-08-22 の絞り込み＝**書式だけが動いたストーリーは一覧に出さない**(ユーザー指定「属性の変更は無視。見つけるのは Text とルビと圏点だけ」)。フォント・色・段落スタイル・表の罫線を変えただけの行は、本文を読みに来た人の前ではノイズになるため。⇒ **一覧に出るのは4種**＝①本文が変わったストーリー ②ルビが変わったストーリー ③圏点が変わったストーリー ④ストーリーごと増えた・消えたもの(Added / Deleted)。⚠**Pixel Changes モードではルビと圏点だけの変更は出ない**＝ルビも圏点も「属性」なので、本文を突き合わせない Pixel モードではカウンター上「書式が動いた」としか読めず、**フォントを変えただけの行と見分けがつかない**(2026-08-22 ユーザー判断＝Pixel では諦めて Text の変更だけを出す。Story Changes モードなら3種とも出る)。★この絞り込みは Story Edits と同じく**公開版 1.3.0 には存在しない機能の仕様**なので、リリースノートに「挙動を変えた」とは書かない ＝ ③の説明としてこう書く。
...
//     ⑨-c ★★**ストーリーの4カウンターは IDML に書き出されない**(2026-08-18)＝スクリプトからは読めるまま、**利用者が書き出す IDML には一切現れない**。プロパティは既定では IDML の属性になる(実測で全 `<Story>` に載っていた)ので、INX 専用のスクリプトリソースで明示的に除外した。**提出説明に書く必要は無い**(利用者から見て何も変わらない)が、**IDML を扱う利用者にとっては「自作プラグインがファイルを汚さない」保証**なので、訊かれたら答えられるようにここに残す。
														...
														//   ⑭★★**マークを「押している間」だけでなく「常に」出せるようにした**(2026-08-22)。フライアウトに **「Show Marks on Target」** を新設(「Show Marks on Source」の直上)。ON の間は、KESCM ツールで左ボタンを押さなくても Target 文書のマークが出たままになる。★**Pixel では比較の枠、Story では変更した文字の反転**＝**同じトグルが両モードで意味を持つ**(描く機構は別々だが、ユーザーから見た意味は1つ)。⚠**画面だけ**＝Target 文書が印刷/PDF に何を出すかは従来どおり「Print comparison marks」が単独で決める(Source 版が印刷にも出るのとは非対称・意図的)。
														//     ★★★**Story Changes モードのマークを新設**＝**変更・追加された文字そのものを反転**して見せる。**枠ではない**ので **拡大率で大きさが変わらず、ページへの外枠も要らない**(2026-08-22 ユーザー指定)。見え方はジャンプ時の一時マーカーと同じ(グローバルテキストアドーンメントの反転)なので、**縦組み・回転フレーム・連結した段にもそのまま乗る**。出し方は2通り＝**トグルで常時**、または**ツールで左ボタンを押している間だけ**(その場合は押した窓の側だけ)。不透明度はパネルの「Marks opacity 25% / 75%」に連動する。
														//     ★**Added / Removed のストーリーは全文が反転する**(相手の版に無いので本文の差分が取れず、全部が新しい／全部が消えたことになるため)。⚠**削除された文字は旧版にしか残っていない**ので、旧版の窓でしか見えない。
