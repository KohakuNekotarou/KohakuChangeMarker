...
//  KCMConstants.h
//
//  ChangeMarker (KCM) のチューニング定数。描画エンジン/peek/色サンプラなど
...
static const PMReal kKCMRingTargetPx = 8.0;	// リングの目標太さ(画面px)。ズームに依らず一定に見せる
...
static const int32 kKCMThumbRingDivisor = 8;
...
static const PMReal kKCMOversetCrossHalfRatio = 0.20;
// ★Find Overset の「＋」の赤線の太さ(ページ短辺 ÷ この値。小さいほど太い)。白縁はこの 2.2 倍。
// 「/」の除数(kKCMThumbDiagDivisor=10)より太く=8(2026-07-24 ユーザー指定=線を少し太く)。
static const int32 kKCMOversetCrossWidthDivisor = 8;
static const uint8 kKCMRingAlpha = 255;	// リングの基本アルファ(0..255)。「通常」=不透明(255)。薄表示は setopacity 側で行う(25%→255×0.25=実25%)
// 枠(リング)の不透明度の二択(パネルのラジオ「Marks opacity 25% / 75%」)。選択値は
// ツール左hold中の画面表示・印刷ON中の常時表示・印刷/PDF出力のすべてに共通で効く(KCMDrawEventHandler::SelectedMarkOpacity)。
static const PMReal kKCMMarkOpacity25 = 0.25;	// 「25%」(薄い)
static const PMReal kKCMMarkOpacity75 = 0.75;	// 「75%」(濃いめ・少し透ける)
...
static const int32 kKCMBaseRadius = 4;	// リング初期半径(画像px)。描画時にズームから再算出するための初期値
static const PMReal kKCMResolution = 36.0;	// 保存・表示のラスタ解像度(dpi)。リング画像/マスクはこの解像度で持つ。
...
// 比較解像度 = kKCMResolution × kKCMHiResMul。低解像度では平均化で消える細線/微小ズレを満額で拾う。
static const PMReal kKCMHiResMul    = 4.0;	// 比較解像度の倍率。★保存36dpi化に合わせ 2.0→4.0(36×4=144dpi で検出力は従来と同一)。
												// 比較時の一時メモリも従来(144dpi)と同じ
static const int32  kKCMPoolMinCount = 1;	// プーリング: 低解像度1セル内の「高解像度の変化画素数」がこの値以上で変化と判定。
...
// 使う側: Start の全/差分比較(KCMCore.cpp)と、Pages パネル選択ページの Refresh(KCMPeek.cpp)。
static const int32  kKCMProgressBarMinPages = 10;

// ★ブック比較(KCMBookCompare.cpp)の進捗バーで、1つの章に割り当てる目盛りの数。
// 分母を「章数」だけにすると、100 ページある章 1 本を処理している間じゅう針が止まり、
// キャンセルを押しても章が終わるまで効かない。章あたり span を割り当てて章の中でも進めることで、
// バーは滑らかに動き、キャンセルの体感も良くなる(KBS が kKBSChapterProgressSpan を 1→10000 に
// 変えて 1851ms→376ms で中断できるようになったのと同じ形)。
// ★値そのものに意味は無い(分母 = 章数 × span で、比だけが効く)。1000 は「1章が 1000 ページを
//   超えなければページ単位で必ず1目盛り以上進む」という目安。
...
static const uint8 kKCMRingR = 255, kKCMRingG = 0,   kKCMRingB = 0;		// 通常(赤)
static const uint8 kKCMRingAltR = 0,   kKCMRingAltG = 255, kKCMRingAltB = 255;	// 赤背景の上(シアン=赤の補色)
...
// 「Check」でチェックしたページに描く ✓ マークの色(青)。Pages パネルのサムネイル中央と、
...
static const PMReal kKCMCheckLayoutSizeRatio   = 0.80;	// ✓ 全体サイズ(ページ短辺比)
static const PMReal kKCMCheckLayoutStrokeRatio = 0.12;	// 線の太さ(✓ サイズ比)
...
static const PMReal kKCMExcludeFillOpacity = 0.35;	// 除外領域ベタ塗りの不透明度(0〜1)
...
static const PMReal kKCMSampleDpi    = 300.0;	// サンプリングのラスタ解像度(dpi)
static const PMReal kKCMSampleHalfPt = 1.0;	// サンプル領域の半幅(pt)。300dpi で約2pt四方≒8px→中心1画素を読む

// ★CMYK カーソル設置後の「落ち着き待ち」(ミリ秒)。カーソルを ICursorMgr::Hide() で隠したまま設置し、
// この時間だけ待ってから Show() する(実装=KCMTracker.cpp の BeginTracking)。0=待たない(既定)。
...
// 見た目: 白フチ+黒文字(背景なし。下の kKCMOldNumR/G/B が正)。バッジ全体の不透明度はパネルの「Marks opacity 25% / 75%」選択に
// 連動(枠と同じ SelectedMarkOpacity() を使う=画面と印刷で一致)。
static const PMReal kKCMOldNumFontPx    = 42.0;	// 文字サイズの基準(px)。実サイズは /kKCMOldNumFixedZoom
static const PMReal kKCMOldNumMarginPx  = 6.0;	// ページ下端から文字下端までの余白の基準(同上)
static const PMReal kKCMOldNumFixedZoom = 0.5;	// ★固定拡大率(ドキュメント50%相当)。sxr の代わりに使う=ズーム/印刷非依存
...
static const PMReal kKCMOldNumHaloEm = 0.06;	// 白フチの太さ(em比)
static const PMReal kKCMOldNumPadEm  = 0.20;	// 透明グループ bbox の余白(em比。白フチのはみ出しを含む)

// (★旧 sprite 版 HUD の点滅対策だった3定数 — kKCMHudSettleTries / kKCMHudSettleDelayMillis /
//  kKCMHudFirstDrawDelayMillis — は 2026-08-06 に**その仕組みごと**撤去した。点滅対策の実測の経緯は
//  git 履歴と docs/ai-notes/kescm-tracker-hud.md に残してある。
//  ⚠**押下中 HUD 自体は現役**＝翌 2026-08-07 に Draw Event 方式で作り直され、待ち時間の定数を
//    必要としない作りになった(UI 側 KCMTrackerHud.cpp)。ここは 2026-08-19(不具合再検査 B-U6)まで
...
// 濃さを変えたいときはここ1箇所を書き換える。★Windows 専用(実体 KCMPanelAlpha.cpp)。
...
static const uint32 kKCMPanelAlphaReapplyDelayMillis = 50;	// 50ms × 8 回 ＝ 約 400ms 追いかける
// (パネルの影(OWL.ShadowView)は alpha ではなく表示/非表示で制御する。影は per-pixel alpha で
//  描かれていて一様 alpha と排他のため、alpha を一度設定すると OFF に戻しても元の影に復帰しない
//  =2026-07-29 に実機で確認。詳細は KCMPanelAlpha.cpp のコメント。よって濃さの定数は持たない)
