...
//  to draw it.
//
//  ★★★MOVED HERE FROM ui/KCMStoryPressMarks.cpp ON 2026-08-23, with the adornment it feeds.
...
//  reads IKCMStoryEditsFacade, and that list has always been the model's.
...
/** Work out what should be lit right now from the toggles, the press and the comparison result,
	...
	(KCMStoryMarker.h keeps the two apart per document).
...
/** The tool's left button went down over one of the two windows, or came back up.
	...
	KCMStoryMarkRefresh. This call says only which window, and the caller does not have to know
...
/** May this document's marks be drawn onto paper or into an exported PDF?
	...
	★★THE RULE IS THE PIXEL MODE'S, WORD FOR WORD (user's choice, 2026-08-23):
	  ...
	  * the OLDER one prints when "Always Show Marks on Source" is on, and does not consult the print
	    toggle at all.
	That asymmetry is not this file's invention - IKCMCompareFacade.h:146-147 already states it as
	...
	The Pixel side answers the same question the same way (KCMRingAdornment.cpp:253).
	⚠**Which document is which is asked of the armed pair** (KCMArmedTargetDB / ...SourceDB), the
	same source the marks themselves were built from. ⚠Not KCMDrawEventHandler::sDB - that is the
