...
//  (A hand-written service provider did this job once; it did exactly what the API's own
//  implementation does.)
//
...
//
//  **THE OFFICIAL SHAPE IS A TWO-STEP, AND IT DOES NOT HAVE THAT PROBLEM.** The argument above
//  only rules out DROPPING on BeforeClose -- it does not rule out LISTENING on it. Adobe's own
//  linksui does precisely that (ClosingDocumentsResponder): on kBeforeCloseDoc it builds a key
//  from the document (path + IDataBase::GetDocumentID) and merely REMEMBERS it; on
//  kAfterCloseDoc -- where its own comment states "Document is null in the signal data at this
//  point of time" -- it uses the remembered key. A cancelled close just leaves a stale key that
//  the next one overwrites, so nothing is dropped early. That names the closed document
//  exactly, with no sweep.
//
...
/** KCMBeforeSaveDocResponder
	...
	**WHY THE SAVE AND NOT THE CLOSE.** Everything else KCM does on a close is dropping its OWN
	state, and that belongs on AfterClose (see the top of this file). Hide Unchanged is the
	exception because it changed the user's document: kHideSpreadCmdBoss is a persistent edit, so
	spreads left hidden are hidden in the saved .indd -- and hidden spreads do not print and do
	not export, which is a page silently missing from whatever gets sent out.

	@warning MEASURED, in this order:
	  ...
	  2. So a kBeforeCloseDoc responder was written -- and it did NOT fix it. A diagnostic build
	     ...
	     save has already happened.** Restoring there put the spreads back on screen and made the
	     document dirty again, while the bytes on disk still had them hidden.
	...
	**The spreads are NOT hidden again afterwards** (the user's call). Re-hiding would need
	...
	disk is always the honest document, and the user turns the toggle back on if they still want
	it.

	**SAVE AS IS DELIBERATELY NOT COVERED, AND THAT IS THE FEATURE.** (The user's call.) Save As
	and Save a Copy raise their own signals (kBeforeSaveAsDocSignalResponderService /
	kBeforeSaveACopyDocSignalResponderService, DocumentID.h), which this responder does not
	answer, so those paths write the document with its spreads STILL HIDDEN. That gives the user a
	clean division:
	...
	**MEASURED, not inferred**: with Hide Unchanged ON, document.save(File) produced a file that
	reopened as [H,-,H,H] -- hidden on disk -- while the same state saved over the original
	reopened as [-,-,-,-]. @warning if a signal is ever added here that division disappears; say
	so first.
*/
...
{
	// @warning restoring runs a command (kHideSpreadCmdBoss with kFalse), and this plug-in is
	...
	// anyway -- but a model change must not be attempted from there at all, and the guard says so
	...
	// hidden with nothing remembering it -- the very state this responder exists to prevent. The
	// other side is usually not being saved in the same breath, but it is still open, and leaving
	// it hidden would just move the problem to its own save. KCMResetHideUnchanged checks liveness
