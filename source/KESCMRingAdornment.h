...
//        (kKESCMRingAdornmentStartupBoss。これで UI の PDF 書き出し＝BG でも載る)。
...
// (★KESCMRingAdornmentIsActive() は 2026-08-20 に撤去した ---- 「今この実行コンテキストでアドーンメントが
...
//    KESCMRingAdornmentRegister() の実装コメントへ移してある。)
...
//     `kKESCMExportXPResponderServiceBoss` の注記。
...
//     `IPrintSetupProvider` を外した。A/B の条件と復活手順は `KESCMRingAdornment.cpp` の節5。
