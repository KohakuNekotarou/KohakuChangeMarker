...
- **REP-05** 作り方＝両側を PDF に書き出し（`kPDFExportCmdBoss`・セッションの PDF 設定・見開き OFF・セキュリティ OFF・UI なし・進捗バーなし・
  書き出し後に開かない）→ 窓なしの新規文書に `kSetPDFPlacePrefsCmdBoss`（ページ番号・Media で切る）＋`SDKLayoutHelper::PlaceFileInFrame` で配置
  → 見出し・注記はテキストフレーム（左揃えを明示）→ 報告書を書き出し → 文書を閉じ、配置設定を元に戻す。
  - ★★★**2026-09-14 改訂＝その2つの PDF は「一時ファイル」をやめ、報告書と同じフォルダーに残す**（ユーザー決定）。
    保存ダイアログで選んだ名前から**`<名前> YYYY-MM-DD HHMMSS\` というフォルダーを作り**、中に3つ置く＝
    `<名前>.pdf`（合体）／`<名前>-before.pdf`（旧）／`<名前>-after.pdf`（新）。**消さない。**
    - ★**日時はここで効く**＝フォルダーには保存ダイアログのような上書き確認が無いので、**名前が重ならないようにするのは自分の仕事**
      （ユーザーの問い「デスクトップにフォルダーをつくるとして重複しない？」）。⚠`ERROR_ALREADY_EXISTS` は成功扱いにしない。
    - ★**理由はディスクでも速度でもない**＝ユーザーの言葉「**勝手につくって勝手に消されるのがいやなので**」。
      **同じ2つのファイルでも、人が場所と名前を決めて残るなら、それは別のもの。**
    - ⚠**「1つもファイルを作らない」道は丸一日かけて全部つぶれた**（メモリ上の PDF＝`kPDFExportItemsCmdBoss` は**重なり順が逆転し半透明が消える**／
      スニペットは**マスターのアイテムとノンブルを運べない**／ページ書き出しをメモリへ出す公開の道は無い）。
      全文＝`docs/ai-notes/kcm-report-without-temp-files-2026-09-14.md`。**再挑戦する前にそれを読むこと。**
...
- **REP-06** 出力先＝**保存ダイアログで選ぶ**（2026-09-13 ユーザー要望「保存場所と名前をつけられるように」。最初は Target の隣に固定だった）。
  初期名は `<Target 名>.compare-report.pdf`。書けたら**既定の PDF ビューアで開く**（開くのは合体した PDF）。
  ★**2026-09-14 改訂**＝選んだ名前は**フォルダー名の元**になる（REP-05）。ステータス行は
  `Report: N pages, with the Before and After PDFs -> <フォルダーのパス>`＝**ファイルではなくフォルダーを示す**（3つ書いたので）。
  キャンセルは `Report cancelled.`、失敗は `Report failed: <理由>`。
  ⚠**失敗しても書けたところまでは残す**（消さない）＝どこまで進んだかが見えるほうが親切。
  - 訂正:
