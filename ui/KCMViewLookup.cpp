...
//  KCMViewLookup.cpp
//
//  レイアウトビューに向かって聞く問い(KCMCore.cpp から分離。2026-08-13 の model/UI 分割
...
//    KCMFindDocDbForView のフォールバックごと畳んだ(下の実測を見よ)。
//
//  UI 側: どの関数も IControlView を受け取るか返すので、model プラグインからは触れない。
//  ★逆流(model 側 KCMColorSampler.cpp / KCMPeek.cpp からの呼び出し)は 2026-08-15 の第2段
...
#include "IPanorama.h"				// KCMQueryPanorama(2026-08-13 に KCMDrawEventHandler.cpp から移動)
...
#include "IKCMCompareFacade.h"	// IsDocDBOpen(公式ルートで得た db の生存確認。2026-08-14 Task 16 で Facade 経由へ)
...
// マウス下のレイアウトビューを求める(Split Window対応)。KCMViewLookup.h のコメント参照。
IControlView* KCMQueryViewUnderMouse()
{
	...
	//   副ペイン側 :362 は kLayoutWidgetID＝**問いに合う方**を使っている。KCM 内でも
	//   KCMScrollMap.cpp が同じ理由で既に kLayoutWidgetID へ寄せてあり、ここだけ残っていた。
	...
	if (pointHit != nil)
	{
		...
		else if (pointHit->GetWidgetID() == kLayoutSecondaryPanelWidgetID)
		{
			...
			//   パネルは panorama を持たないので、そのまま返すと KCMQueryPanorama が親を辿って
...
// view がどの文書のレイアウトビューかを特定する。KCMViewLookup.h のコメント参照。
// (2026-07-25: KCMPeek.cpp の file-static から共有ヘルパへ移動。色サンプラの窓ガードでも使うため)
// ★★★2026-08-16: そのビューが今表示しているスプレッド(KCMViewLookup.h に理由の全文)。
//   上の KCMFindDocDbForView と同じ boss・同じインターフェイスに、別の問いをするだけ。
...
IDataBase* KCMFindDocDbForView(IControlView* view)
{
	...
	//   「その文書がまだ開いているか」は分からないので、ここで確認して KCM 全体の規約
...
//
// ★2026-08-13 に KCMDrawEventHandler.cpp から移した(model/UI 分割 第1段 Task 12)。中身は無変更。
