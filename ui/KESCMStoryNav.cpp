...
// Project includes:
#include "KCMUIID.h"				// kKESCMStoryTreeWidgetID
#include "KESCMStoryNodeID.h"		// the tree's node class: (row, change)
#include "KESCMStoryJump.h"			// what a stop DOES - the row click's own behaviour
#include "KESCMUIShared.h"			// KESCMGetVisibleOwnPanel
#include "IKESCMStoryEditsFacade.h"	// the list itself, read across the model/UI boundary
#include "KESCMStoryNav.h"
...
/** Put the list's own selection on this stop and bring it into view.
	...
	row (KESCMStoryTreeEH) once they click into the list.
	...
	does not depend on any of them (KESCMStoryTreeRebuild is written the same way and says so).

	⚠THE KEY FOCUS IS DELIBERATELY LEFT ALONE, where a row click takes it (KESCMStoryRowEH's
...
*/
void SelectInTree(const KESCMStoryNavStop& stop)
{
	...
	IControlView* treeView = panelData->FindWidget(kKESCMStoryTreeWidgetID);
	if (treeView == nil)
		return;
	...
	if (stop.fChange >= 0)
		treeMgr->ExpandNode(KESCMStoryNodeID::CreateStory(stop.fRow), kFalse /*expandAllDescendants*/);

	const NodeID node = (stop.fChange >= 0)
						? KESCMStoryNodeID::CreateChange(stop.fRow, stop.fChange)
						: KESCMStoryNodeID::CreateStory(stop.fRow);
...
//----------------------------------------------------------------------------------------
// KESCMBuildStoryNavStops (KESCMStoryNav.h)
//----------------------------------------------------------------------------------------
void KESCMBuildStoryNavStops(std::vector<KESCMStoryNavStop>& out)
{
	...
	// ⚠AND IT IS TESTED FOR nil, unlike the calls in KESCMStoryJump.cpp. The difference is the
	//   caller: those run from a click, so the panel is up and kUtilsBoss is certainly there, while
	//   this is also reached from a model NOTIFICATION (KESCMRefreshNavPosition, called when the
...
//----------------------------------------------------------------------------------------
// KESCMGotoStoryNavStop (KESCMStoryNav.h)
//----------------------------------------------------------------------------------------
void KESCMGotoStoryNavStop(const KESCMStoryNavStop& stop)
{
	...
	//   KESCMNoteStoryStop, which they call as soon as the row is known to exist. That is what lets
	...
	if (stop.fChange >= 0)
		KESCMStoryJumpToChange(stop.fRow, stop.fChange);
	else
		KESCMStoryJumpToRow(stop.fRow);
}

// End, KESCMStoryNav.cpp.
