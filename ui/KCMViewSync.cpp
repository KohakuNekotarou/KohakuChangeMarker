...
//  KCMViewSync.cpp
//
//  レイアウトビュー同期の実装(KCMPeek.cpp から分離。2026-08-13 の model/UI 分割 第1段 Task 1)。
...
//    ⚠2026-08-17 訂正: 分離当初は「KCMCore.h が公開しているアクセサ(KCMIsArmed 等)で読む」と
//    書いていたが、それは第1段 Task 11 で Facade 経由に変わっている。**同名の関数は model 側に今も
//    実在する**(KCMCore.h が `KCMIsArmed` を宣言し、KCMPeek.cpp の「パネルの状態アクセサ」の段が
//    定義している)ので、この記述を残すと「UI が model の関数を直に呼んでいる」= 分割の一方向依存が
//    破れている、と誤読させる。
//    ⚠2026-08-18(不具合再検査 B10 の2周目): ここは行番号(KCMCore.h:154 / KCMPeek.cpp:686)で
...
#include "KCMViewLookup.h"         // KCMFindDocDbForView(2026-08-13 に KCMCore.h から移動)
                                     // ＋ KCMQueryPanorama(2026-08-13 に KCMDrawEventHandler.h から移動)
#include "IKCMMarkData.h"          // GetPagePairing / GetMasterPagePairing(除外対応表)。
                                     //   2026-08-13 Task 13 で KCMPageMap.h から移した
#include "KCMChangeNav.h"          // KCMEnsureViewShowsSpread(同期先ビューを相手のスプレッドへ。2026-08-11)
...
// (KCMFindDocDbForView は 2026-07-25 に KCMCore.cpp の共有ヘルパへ移動し、2026-08-13 の分割
//  第1段 Task 3 で KCMViewLookup.cpp へ移った。**宣言は KCMViewLookup.h**(上の include 参照)。
//  ⚠2026-08-17 訂正: ここは「宣言は KCMCore.h」のままだった＝B-U3 が UI 側6箇所で直したのと
//  同じ型の壊れた参照。ui/ に KCMCore.h は存在しない)
...
//     ・除外対応表の再構築(KCMBuildPairing = 両文書の全ページ走査 + 登録判定)
//   をやり直していた。ページ数に比例した仕事が1通知ごとに乗るので、長い文書ほど追従が重くなる。
//   これらはいずれも「スクロールしている間は変わらない」ものなので、短時間だけ覚えて使い回す。
//
//   無効化は2本立て:
//     ①明示 … KCMInvalidateSyncCaches()(arm/disarm・同期 OFF・文書クローズ・Shutdown)
//     ②時間 … kKCMSyncCacheTtlMs(250ms)経過で自動失効。ページの追加/削除やスプレッドの
...
// 同期キャッシュを丸ごと捨てる(KCMViewSync.h で宣言)。
void KCMInvalidateSyncCaches()
{
	...
	// (2026-08-17: ここで KCMForgetViewDbHint() も呼んでいたが、view→db のヒントごと
	//  フォールバックが畳まれて無くなった＝KCMFindDocDbForView は毎回ビュー自身に聞く)
}
...
static bool16 KCMPagePasteboardRectRaw(IDataBase* db, UID pageUID, PMRect& outPB)
{
	...
	//   ブロック10 で ChangeNav を寄せたときの論点が、ここと KCMCore/KCMScrollMap に残っていた)。
...
static const KCMPageRectCache* KCMGetPageRects(IDataBase* db)
{
	...
	c.pages.clear();
	c.rects.clear();
	// ★2回続けて聞くので InterfacePtr に1回受ける(Utils.h:74-80。下の KCMEnsureSyncPairing と同じ形)。
...
static UID KCMFindPageAtPasteboard(IDataBase* db, const PBPMPoint& pb)
{
	...
	//   その判定は呼び出し側(KCMCorrectedCenterForDoc)が SDK の答えで行う。
	for (size_t i = 0; i < c->normalCount; ++i)
	{
		...
		//   手本は snapshot/SnapTracker.cpp:600,617 で、**上の KCMPagePasteboardRectRaw が矩形取得の
		//   手本として引いている :610-616 の直後**にある。model 側 KCMCore.cpp は 2026-08-16 の B3 で
		//   既に寄せてあり、ここだけが手書きの L/R/T/B 比較で残っていた。
		//   ⚠PointIn は素の閉区間比較なので非正規化の箱では常に kFalse になるが、この表の矩形は
		//   KCMPagePasteboardRectRaw が min/max を取って正規化済みなので Normalize() は要らない。
		if (r.PointIn(pb))
...
// ★旧実装は KCMMapTargetToSource / KCMMapSourceToTarget が呼ばれるたびに KCMBuildPairing を
...
// (KCMMapTargetToSource / KCMMapSourceToTarget=登録ページを除外して残りを順番対応させるので、
                                            ...
                                            const PBPMPoint& srcCenter, bool16& outSkip, UID& outDstPage)
{
	...
	// KCMMapTargetToSource/SourceToTarget と同一で、「対応表に無い=相手なし」を skip として扱う点も同じ。
	...
	// ★まず公式ルート(KCMQueryViewCenterPage)に聞く(2026-08-06 の監査 A-2)。
	// ★★公式ルートは「文書のどのページか」を広く答えるので、マスタースプレッドのページも返ってくる。
	//   2026-08-11 からは**マスターも対応表に載っている**(KCMEnsureSyncPairing が名前対応を足す)ので、
	...
	//   (★KCMCollectPageUIDs 自体は比較ペアリングと共用のため変えない)。
...
//   使う(公式ルート=KCMQueryViewCenterPage。2026-08-06 の監査 A-2)。nil でも動く(自前探索へ落ちる)。
//
// ★★★戻り値=「複製先のビューが1つでもあったか」(2026-08-19・不具合再検査 B-U7)。
//   **「そろえたか」を答えられるのはこの関数だけ**なので、ここが答える。以前は void で、唯一それを
//   知りたい呼び手(KCMAlignOtherViewsToActiveNow)が**この関数のガードを手で写した先読み**を持ち、
                                           ...
                                           bool16 applyPageOffset = kFalse)
{
	...
	//       ★2026-07-24 変更: 旧仕様は「Stop 中は KCM ツール選択中のみ同期」(誤同期回避が目的)だったが、
	//       この関数に来る時点で ONトグル(sLayoutSyncOn)は必ず ON 確定なので、それを作動条件と見なし、
	//       ツールの選択状態に関係なく同期する(ユーザー指定)。
	InterfacePtr<IKCMCompareFacade> compare(Utils<IKCMCompareFacade>().QueryUtilInterface());	// ★1回引いて使い回す(Utils.h:74-80)
	...
	if (!stopBroadSync)
	{
		...
		// ★FindDocByDataBase へのポインタ比較のみ=閉じた db を deref しない(KCM 共通規約)。
	...
	//   GlobalErrorStatePreserver で、KCM でも BookCompare / BookOpen / HideUnchanged(B10)は採用済み
	//   ＝ここと KCMChangeNav が取り残されていた。★broadcastGuard より先に作る＝あとから作った方が
	...
	{
		KCMSyncBroadcastGuard()  { sLayoutSyncBroadcasting = kTrue; }	// ここからの通知は自分発なのでオブザーバは無視する
...
//   スクロール/ズームしたら KCMSyncOtherDocViewportsTo で他文書のビューへ複製する(自動・ライブ)。
...
// ★実証済み構成: .fr の AddIn で kActiveContextBoss に IID_IKCMLAYOUTSYNCOBSERVER として同居させる
...
// (CreateObject2)に、boss上に存在しない IID_IKCMLAYOUTSYNCOBSERVER を asObserver として渡していた
// ため Update が届かなかった。現構成は実装が実際に IID_IKCMLAYOUTSYNCOBSERVER で載っており整合する。
static IObserver* KCMQueryLayoutSyncObserver()
{
	// session は終了処理中に nil になり得る(2026-07-25 追補 に KCM 全体で統一)。
...
/** レイアウトビュー同期オブザーバの実装。.fr の AddIn で kActiveContextBoss に
    IID_IKCMLAYOUTSYNCOBSERVER として同居させている(手本と同じ実証済み構成)。 */
...
public:
	...
	//   .fr の AddIn は IID_IKCMLAYOUTSYNCOBSERVER で載せ、Attach もその IID で行うので、
...
void KCMLayoutSyncObserver::Update(const ClassID& theChange, ISubject* theSubject, const PMIID& protocol, void* changedBy)
{
	...
	KCMSyncCacheBeginTick();	// TTL 超過ならこの中でキャッシュ一式(前回状態を含む)が捨てられる
...
// KCMGetLayoutSync / KCMSetLayoutSync(KCMViewSync.h で宣言) — フライアウトトグルの実体。
...
void KCMSetLayoutSync(bool16 on)
{
	...
	KCMInvalidateSyncCaches();	// ON/OFF のどちらでも、次の同期は最新の実測から始める(2026-07-25 追補)
	...
	if (on)
	{
		...
		if (front != nil)
		{
			...
			if (pano != nil && db != nil)
				KCMSyncOtherDocViewportsTo(front, pano, db, kTrue /*applyPageOffset(本仕様): 初回そろえも補正付き*/);
		}
	}
	else
	{
...
// KCMAlignOtherViewsToActiveNow(KCMViewSync.h で宣言) — フライアウト/ショートカットの実行アクション。
// アクティブ(最前面)レイアウトビューの位置+拡大率を他文書の全ビューへ1回そろえる。Sync Layout Views
// トグルの ON/OFF とは独立で、OFF でも押せば1回だけそろう(トグル ON 時の初回そろえ=上の KCMSetLayoutSync
// と同じ手本ビュー取得+同じ同期エンジン呼び出し)。applyPageOffset=kTrue を渡すので、Start 中(arm)は
// KCMSyncOtherDocViewportsTo のガードにより Target↔Source 間でページの Add/Remove 補正が掛かり(賢く一致)、
...
bool16 KCMAlignOtherViewsToActiveNow()
{
	...
	return KCMSyncOtherDocViewportsTo(front, pano, db, kTrue /*applyPageOffset(arm時のみ補正・未arm時は関数内でkFalse強制)*/);
}

// KCMViewSyncShutdown(KCMViewSync.h で宣言) — 終了時の後始末。
void KCMViewSyncShutdown()
{
	// レイアウトビュー同期の後始末は「状態フラグを落とすだけ」にする(以後の通知は Update 先頭の
	// ガードで無視される)。★KCMSetLayoutSync(kFalse) をここで呼んではならない: その経路は
	...
	//   **GetActiveContext() は終了処理中に触っても落ちない** ---- KCMDetachModelChangeObserver と
	//   KCMDetachPanelVisibilityObserver が終了時にまさにそれを呼んでおり、Task 13 の終了安全性
	//   (書き出し10本キュー直後の終了を含む)で PASS している。
	// ⇒ ★**危険物を並べて書くと、そのうち1つだけを使う操作まで禁止に見える。**
	//   実際この2つ並びが KCMPeekGesture.cpp で「**detach 自体がクラッシュ要因**」という
