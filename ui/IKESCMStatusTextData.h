...
//  clicked, this box shows the OTHER side of that edit (KESCMStoryJump.cpp), and the reader's
...
//  what the stock widget drew. That is why the 72 places that call KESCMSetStatus needed no change.
//
...
//  what has to outlive the panel is remembered on the MODEL side instead (KESCMModelNotify.cpp),
//  which is also where app.kcmStatus answers from.
//
//  ★Same shape and the same reasoning as IKESCMStoryCellData, which feeds the change row's cell.
//  Four strings rather than three: a row has no room for a heading and no need of one.
//
//========================================================================================

#ifndef __IKESCMStatusTextData_h__
#define __IKESCMStatusTextData_h__
...
// Project includes:
#include "KCMUIID.h"		// IID_IKESCMSTATUSTEXTDATA

/** Holds what the panel's message area draws, split where the colour changes. */
class IKESCMStatusTextData : public IPMUnknown
{
public:
	enum { kDefaultIID = IID_IKESCMSTATUSTEXTDATA };
