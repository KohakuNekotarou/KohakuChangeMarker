...
//                                        ("pixel"/"story")。理由は KESCMPanelState.cpp の
//                                        KESCMJsonReadString の頭(2026-08-21)
//
...
//      `source/KESCMID.h` の増分⑱に書いてある。
//    - Start/Stop の arm 状態、Hide Unchanged、Find Overset …… 「その瞬間の作業状態」(上記のとおり)。
//    - ページごとの ✓(KCM: Check)と地図 …… これは**別の永続化**で、保存先も別(KESCMPageCheck.cpp)。
//
...
//    ⇒ 点検の仕方＝`KESCMActionComponent.cpp` の `UpdateActionStates` に並ぶ**チェック式/ラジオ式の
...
// ★呼び出しタイミングは起動時(KESCMUIStartup::Startup。2026-07-15 に前倒し): 同期が Stop 中+
