...
//  through EVERY row, parents included (KCMStoryTreeEH), because there the reader is moving
...
//  built where they always were (KCMChangeNav.cpp). This file is asked only when the mode is
//  kKCMModeStory.
//
//========================================================================================

#ifndef __KCMStoryNav_h__
#define __KCMStoryNav_h__
...
/** One stop of the Story Changes walk.
	...
	★IDENTIFIED BY ALL THREE TOGETHER - the story, the row and the change (KCMChangeNav.cpp's
	KCMFindCurrentStop). The navigation remembers where it is by CONTENT rather than by index
	...
	⚠THE ROW NUMBER WAS ADDED ON A REASON THAT TURNED OUT TO BE WRONG (2026-08-25, and withdrawn the
	  ...
	  target row and on a source's deleted one". They cannot - KCMStoryStamp.h defines Added as
...
/** Fill out with every leaf of the Story Edits list, in the order the list is in.
	...
	order with the removed ones grouped after them (KCMStoryList::Build), and a reader pressing
	...
	story: both leave KCMStoryList empty, and the caller then has no stops of this kind to add.
*/
...
/** Go to one stop: exactly what clicking that row in the list does, plus moving the list's own
	...
	★THE JUMP IS THE CLICK'S (KCMStoryJump). A change stop calls KCMStoryJumpToChange - both
	windows move to the edit, a mark flashes over the characters, and the other side's wording goes
	to the message area. A parent stop calls KCMStoryJumpToRow - the story's beginning, and the
	...
	★AND THE WALK'S POSITION IS MOVED BY THE JUMP, not by the caller (KCMNoteStoryStop, called
