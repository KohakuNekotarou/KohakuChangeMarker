...
//  same base, as the panel's Story Edits list (KCMStoryTreeAdapter.cpp).
...
*/
class KCMBookTreeAdapter : public ListTreeViewAdapter
{
public:
	KCMBookTreeAdapter(IPMUnknown* boss) : ListTreeViewAdapter(boss) {}
	virtual ~KCMBookTreeAdapter() {}

protected:
	virtual int32 GetNumListItems() const
	{
		return static_cast<int32>(KCMBookDialogRows().size());
	}
};

CREATE_PMINTERFACE(KCMBookTreeAdapter, kKCMBookTreeAdapterImpl)

// End, KCMBookTreeAdapter.cpp.
