...
//  messages through KESCMSetStatus and several of them put a full save path in here - so it is
...
//  72 callers needed no change (IKESCMStatusTextData.h).
//
...
//  (KESCMStoryCellView.cpp), so that the two answer that question the same way. The change itself
...
#include "KCMUIID.h"
#include "KESCMPanelTextDraw.h"	// kKESCMContextTextWeight, KESCMBlendColor - shared with the row cell
...
const bool16 kKESCMDontConvertAmpersand = kFalse;
const bool16 kKESCMNoUnderline = kFalse;
...
PMString KESCMHead(const PMString& s, int32 n)
{
	PMString out(s);
...
PMString KESCMTail(const PMString& s, int32 n)
{
	PMString out(s);
	if (n > 0)
...
/* KESCMSafeCut
   ...
   ★It cuts BEFORE the pair, whichever side is being kept: KESCMHead keeps [0, pos) and KESCMTail
...
int32 KESCMSafeCut(const PMString& s, int32 pos)
{
...
/** U+2026 HORIZONTAL ELLIPSIS, the same character the model puts on a cut excerpt
	(KESCMStoryDiffRun.cpp writes it as the UTF-8 bytes \xE2\x80\xA6).
...
PMReal KESCMWidth(IGraphicsContext* gc, const PMString& s, const InterfaceFontInfo& font)
{
	if (s.IsEmpty())
		return PMReal(0.0);
	return StringUtils::PMMeasureString(gc, s, font, kKESCMDontConvertAmpersand).X();
}
...
int32 KESCMFitCount(IGraphicsContext* gc, const InterfaceFontInfo& font,
					const PMString& s, const PMReal& room)
{
	const int32 total = s.CharCount();
	if (total <= 0 || room <= PMReal(0.0))
		return 0;
	if (KESCMWidth(gc, s, font) <= room)
		return total;
	...
	while (lo < hi)
	{
		...
		if (KESCMWidth(gc, KESCMHead(s, mid), font) <= room)
			lo = mid;
		else
			hi = mid - 1;
	}
...
bool16 KESCMAnythingLeft(const std::vector<KESCMRun>& runs, size_t atRun, const PMString& rest)
{
	if (!rest.IsEmpty())
		return kTrue;
...
bool16 KESCMLayoutRuns(IGraphicsContext* gc, const InterfaceFontInfo& font,
					   const std::vector<KESCMRun>& runs, const PMReal& availWidth,
					   int32 maxLines, std::vector<KESCMFrag>& out)
{
	out.clear();
	...
	for (size_t r = 0; r < runs.size(); ++r)
	{
		...
		while (!rest.IsEmpty())
		{
			...
			if (rest.GetChar(0).IsLineBreakChar())
			{
				...
				if (line >= maxLines)
					return !KESCMAnythingLeft(runs, r, rest);
				continue;
			}
			...
			const PMString chunk = (breakAt < 0) ? rest : KESCMHead(rest, breakAt);

			const PMReal room = availWidth - x;
			int32 fit = KESCMFitCount(gc, font, chunk, room);
			...
			else
			{
				...
				fit = (space > 0) ? space : KESCMSafeCut(chunk, fit);
			}
...
std::vector<KESCMRun> KESCMMakeRuns(const PMString& label, const PMString& pre,
									const PMString& mid, const PMString& post)
{
	std::vector<KESCMRun> runs;
	if (!label.IsEmpty())
	{
...
PMString KESCMTrimLeadingContext(const PMString& pre, int32 keep)
{
	const int32 total = pre.CharCount();
	if (pre.IsEmpty() || keep >= total)
		return pre;
	PMString out = KESCMEllipsis();
	out.Append(KESCMTail(pre, KESCMSafeCut(pre, total - keep)));
...
/** The trailing context cut down to its first `keep` characters. It loses its TAIL, on the same
	terms as above. */
PMString KESCMTrimTrailingContext(const PMString& post, int32 keep)
{
	const int32 total = post.CharCount();
	if (post.IsEmpty() || keep >= total)
		return post;
	PMString out = KESCMHead(post, KESCMSafeCut(post, keep));
	out.Append(KESCMEllipsis());
	out.SetTranslatable(kFalse);
	return out;
}
...
//----------------------------------------------------------------------------------------
// KESCMStatusTextData - the four strings the message area draws
//----------------------------------------------------------------------------------------

/** Non-persistent holder for the current message, aggregated on the widget's boss beside the view.
	Written by KESCMSetStatus / KESCMSetStatusSegments, read by Draw. */
class KESCMStatusTextData : public CPMUnknown<IKESCMStatusTextData>
{
public:
	KESCMStatusTextData(IPMUnknown* boss) : CPMUnknown<IKESCMStatusTextData>(boss) {}
...
//----------------------------------------------------------------------------------------
// KESCMStatusTextView - the self-drawing message area
//----------------------------------------------------------------------------------------

/** Implements IControlView: wraps the message into the box and draws it in up to two colours. */
class KESCMStatusTextView : public DVControlView
{
	typedef DVControlView inherited;
public:
	KESCMStatusTextView(IPMUnknown* boss) : inherited(boss) {}
	virtual ~KESCMStatusTextView() {}

	virtual void Draw(IViewPort* viewPort, SysRgn updateRgn);
};

CREATE_PERSIST_PMINTERFACE(KESCMStatusTextView, kKESCMStatusTextViewImpl)

void KESCMStatusTextView::Draw(IViewPort* viewPort, SysRgn updateRgn)
{
	...
	InterfacePtr<IKESCMStatusTextData> data(this, UseDefaultIID());
	if (data == nil)
		return;
	...
	if (!haveMetrics)
	{
		lineHeight = StringUtils::PMMeasureString(&gc, PMString("Ag"), fontInfo, kKESCMDontConvertAmpersand).Y();
	...
	std::vector<KESCMFrag> frags;
	if (!KESCMLayoutRuns(&gc, fontInfo, KESCMMakeRuns(label, pre, mid, post), availWidth, maxLines, frags))
	{
		...
		if (maxKeep > 0)
		{
			...
			while (lo <= hi)
			{
				...
				std::vector<KESCMFrag> trial;
				const bool16 fits = KESCMLayoutRuns(&gc, fontInfo,
					KESCMMakeRuns(label, KESCMTrimLeadingContext(pre, keep), mid,
								  KESCMTrimTrailingContext(post, keep)),
		...
		if (!found)
		{
			...
			while (lo <= hi)
			{
				...
				{
					midCut = KESCMHead(mid, KESCMSafeCut(mid, keep));
					midCut.Append(KESCMEllipsis());
					midCut.SetTranslatable(kFalse);
				}
				std::vector<KESCMFrag> trial;
				const bool16 fits = KESCMLayoutRuns(&gc, fontInfo,
					KESCMMakeRuns(label, kNothing, midCut, kNothing), availWidth, maxLines, trial);
				if (fits)
				{
					best.swap(trial);
					found = kTrue;
