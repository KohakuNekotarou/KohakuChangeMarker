...
//
//  Book comparison: the dialog. See KCMBookDialog.h for why it is modeless and fixed-size.
//
...
// Project includes:
#include "KCMBookDialog.h"
#include "KCMBookOpen.h"		// KCMBookSetMenuRow - the stashed row index, dropped when the rows change
#include "KCMBookTree.h"		// KCMBookTreeRebuild - the list, redrawn when the dialog opens
#include "KCMUIID.h"
#include "KCMPanelAlpha.h"	// KCMSetBookDialogWindow / KCMApplyBookDialogTranslucency
#include "KCMPathDisplay.h"	// KCMPathForDisplay - the "/" separators, shared with the panel

// *windows.h goes AFTER the SDK headers, so its macros cannot collide with SDK names.
//  (The same order KCMPanelAlpha.cpp uses.)
#ifdef WINDOWS
#include <windows.h>
#endif
...
/** Everything the dialog shows, held for it. ★Module scope, not members of the dialog: the dialog
	...
	★The four are written together (KCMBookDialogSetResult) because they describe ONE run - see
	  the header for why the paths are stored rather than resolved again at paint time. */
std::vector<KCMChapterResult> gDialogRows;
...
/** Paint the three text lines from what the last run stored.
	...
	are in KCMOpenBookDialog. */
void KCMBookDialogPaintResult(IPanelControlData* panelData)
{
	if (panelData == nil)
		return;
	...
	// ★What actually holds the width is kKCMBookPathTextWidgetBoss (KCMUI.fr): EVE asks a widget's
	...
	targetLine.Append(KCMPathForDisplay(gTargetPath));
	targetLine.SetTranslatable(kFalse);

	PMString sourceLine("Source: ");
	sourceLine.Append(KCMPathForDisplay(gSourcePath));
	sourceLine.SetTranslatable(kFalse);

	SetLine(panelData, kKCMBookTargetTextWidgetID, targetLine);
	SetLine(panelData, kKCMBookSourceTextWidgetID, sourceLine);
	...
	if (!gSummary.IsEmpty())
		SetLine(panelData, kKCMBookStatusTextWidgetID, gSummary);
}
                              ...
                              const std::vector<KCMChapterResult>& rows)
{
	...
	// popped over (KCMBookOpen.cpp's gMenuRow) because the action it raises is handed no widget
	...
	//   comparison was re-run - and firing kKCMBookRowStartActionID by ID reported enabled=true and
	...
	//   aftermath flag has to reset this the way Clear() does." KCMBookOpen.h claimed this file's
	...
	{
		if (rows[i].fState != kKCMChapterNoChange)
...
void KCMBookDialogShutdown()
{
	...
	// KCMChapterResult carries PMStrings of its own - are emptied so that the static destructors
	...
	//   reason as KCMStoryList::ShutdownCleanup.
	...
	//   the same omission on this side - nothing in KCMUIStartup::Shutdown named them, so a
	...
	//   KCMBookOpen.h asks anything that replaces or drops rows to call KCMBookSetMenuRow(-1)
	//   as KCMBookDialogSetResult does, and that rule earns its keep because a stale index can
	//   land on a VALID row of the next result set. Here there is no next result set: this runs
	//   from KCMUIStartup::Shutdown, after which no menu is built, no action is dispatched and no
	...
	gDialogRows = std::vector<KCMChapterResult>();
	gTargetPath.Clear();
	gSourcePath.Clear();
	gSummary.Clear();
}
...
/** The dialog's controller: it paints what the last run left behind, and applies nothing.
    ...
    ***** There is nothing to apply. ***** Since 2026-08-12 this dialog has no controls at all - the
    comparison is started from the flyout menu and confirmed in an alert (KCMBookRun.cpp), and
...
class KCMBookDialogController : public CDialogController
{
public:
	KCMBookDialogController(IPMUnknown* boss) : CDialogController(boss) {}
...
CREATE_PMINTERFACE(KCMBookDialogController, kKCMBookDialogControllerImpl)

void KCMBookDialogController::InitializeDialogFields(IActiveContext* context)
{
	CDialogController::InitializeDialogFields(context);

	InterfacePtr<IPanelControlData> panelData(this, UseDefaultIID());
	KCMBookDialogPaintResult(panelData);
	...
	KCMBookTreeRebuild(panelData);
}

void KCMBookDialogController::ApplyDialogFields(IActiveContext* context, const WidgetID& widgetId)
{
...
/** Prepare the dialog's own platform window: give it a MINIMIZE BOX, and bring it back when it is
    ...
    *****NOTHING IS PUT BACK, AND THAT IS THE POINT OF DOING IT HERE.***** KBS does the same thing to
    ...
    This one is OURS: it is created by KCMOpenBookDialog and destroyed when the dialog closes, so
...
void KCMPrepareBookDialogWindow(IDialog* dialog)
{
#ifdef WINDOWS
	if (dialog == nil)
		return;
	...
	//   ★The file already said so 60 lines up: "it is created by KCMOpenBookDialog and destroyed
	...
	//   that one too. What makes closing safe is that KCMSetBookDialogWindow also records THIS
	//   window's title and every later use is checked against it (KCMPanelAlpha.cpp, 2026-08-13) -
	...
	KCMSetBookDialogWindow(hwnd);
	KCMApplyBookDialogTranslucency();
#endif
}
...
void KCMOpenBookDialog()
{
	...
	(
		...
		kViewRsrcType,
		kKCMBookDialogRsrcID,			// The dialog's view resource
	...
	//   controller's InitializeDialogFields is NOT called a second time. So KCMBookDialogSetResult
	...
	// ★THE SECOND ONE IS WHY THIS MATTERS. Row indices index KCMBookDialogRows(), which had
	...
	KCMPrepareBookDialogWindow(dialog);
}

// End, KCMBookDialog.cpp.
