...
//    ⇒ 素の名前は `kKESCMDisplayName`（表示名の唯一の定義）から組み立てる。
//
//  ★**呼ぶのは3か所**（どれも「今の状態を書き直す」だけなので、いつ何度呼んでもよい）:
//     ① モードを切り替えたとき（KESCMActionComponent の KESCMApplyCompareMode）
//     ② パネルが表示されたとき（KESCMPanelObserver::AutoAttach。widget は毎回作り直されるが、
//        **タブのラベルはパレットの持ち物なので消えない** ---- ただし初回表示までは書く先が無い
//        ＝下の関数は panelView==nil で黙って戻るので、そのときの一手がここになる）
//     ③ 終了時に素へ戻す（KESCMUIStartup の Shutdown）
//
//========================================================================================

#ifndef __KESCMPanelTitle_h__
#define __KESCMPanelTitle_h__

namespace KESCMPanelTitle
{
