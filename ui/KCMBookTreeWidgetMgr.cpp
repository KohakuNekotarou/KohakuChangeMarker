...
// directory - the same reasoning, and the same route, as KCMStoryTreeWidgetMgr.cpp: the build
...
*/
class KCMBookTreeWidgetMgr : public CTreeViewWidgetMgr
{
public:
	...
	KCMBookTreeWidgetMgr(IPMUnknown* boss) : CTreeViewWidgetMgr(boss, kList) {}
	virtual ~KCMBookTreeWidgetMgr() {}
	...
	virtual IControlView* CreateWidgetForNode(const NodeID& /*node*/) const
	{
		...
		IPMUnknown* newObject = ::CreateObjectNoInit(
			::GetDataBase(this),
			RsrcSpec(LocaleSetting::GetLocale(), kKCMUIPluginID, kViewRsrcType, kKCMBookRowRsrcID),
			IID_ICONTROLVIEW);
	...
	{
		return PMReal(kKCMBookRowHeight);
	}
	...
	virtual bool16 ApplyDataToWidget(const NodeID& node, IPanelControlData* widgetList, int32 /*message*/) const
	{
		...
		if (known)
		{
			...
			//   (kKCMNoTipImpl, deliberately - 2026-08-10), so there was no second way to read it.
			...
			//   spell out that something failed (KCMBookResult.h's fWhy carries the rule).
			//   ⇒ Whatever is appended here must fit next to a file name in ~38 characters.
			if (row.fState == kKCMChapterFailed && !row.fWhy.IsEmpty())
			{
				name.Append(" - ");
...
CREATE_PMINTERFACE(KCMBookTreeWidgetMgr, kKCMBookTreeWidgetMgrImpl)

//----------------------------------------------------------------------------------------
// KCMBookTreeRebuild - redraw the list from the rows the module holds
//----------------------------------------------------------------------------------------
...
void KCMBookTreeRebuild(IPanelControlData* panelData)
{
	...
	InterfacePtr<ITreeViewMgr> treeMgr(panelData->FindWidget(kKCMBookTreeWidgetID), UseDefaultIID());
	if (treeMgr == nil)
		return;
