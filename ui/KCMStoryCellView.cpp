...
//
//  ★WHERE THE THREE PIECES COME FROM: the model splits them (KCMStoryDiffRun's Slice) and hands
//  them across the boundary on IKCMStoryEditsFacade::Change. The split cannot be made here - the
...
//----------------------------------------------------------------------------------------
// KCMStoryCellData - the three strings the cell draws
//----------------------------------------------------------------------------------------
...
class KCMStoryCellData : public CPMUnknown<IKCMStoryCellData>
{
public:
...
//----------------------------------------------------------------------------------------
// KCMStoryCellView - the self-drawing cell
//----------------------------------------------------------------------------------------

/** Implements IControlView: draws one change with its context faded around it. */
class KCMStoryCellView : public DVControlView
{
	typedef DVControlView inherited;
public:
	KCMStoryCellView(IPMUnknown* boss) : inherited(boss) {}
	virtual ~KCMStoryCellView() {}

	virtual void Draw(IViewPort* viewPort, SysRgn updateRgn);
};

CREATE_PERSIST_PMINTERFACE(KCMStoryCellView, kKCMStoryCellViewImpl)

void KCMStoryCellView::Draw(IViewPort* viewPort, SysRgn updateRgn)
{
	...
	// in the .fr (KCMStoryRowCellWidget names kPaletteWindowSystemScriptFontId for both its
	...
	const bool16 isHilited = KCMViewOrParentIsHilited(this, kKCMHiliteParentSteps);
	...
	if (twoLines && !ruby.IsEmpty())
	{
		...
		const PMReal rubyX = KCMRubyX(drawnMidX, drawnMidW, rubyW, leftEdge);
