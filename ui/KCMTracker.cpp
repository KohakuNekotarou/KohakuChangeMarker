...
//    "Not comparing" の1行を出す機能)は**現役**。このファイルが KCMTrackerHudBegin / End を呼び、
//    描く実体は Draw Event 経路(KCMTrackerHud.cpp / KCMTrackerHud.h)。
...
//      経緯と実測は docs/ai-notes/kescm-tracker-hud.md、仕様は KCMTrackerHud.h の冒頭。
//
//========================================================================================
...
#include "KCMUIID.h"
#include "KCMConstants.h"	// kKCMCursorSettleMillis(設置後の落ち着き待ち)
#include "KCMPeekGesture.h"	// KCMClassifyGesture / KCMTrackerRevealBegin / KCMTrackerRevealEnd
#include "KCMCmykCursor.h"	// CMYK カーソル入口(HasPending / CursorProc / UpdateCmykDrag)
#include "KCMTrackerHud.h"	// 押下中だけビュー左上に Target/Source を出す(描画は Draw Event 側)
...
/** ICursorMgr::Hide() → Show() の対を RAII で保証する(2026-07-30 の監査で追加)。
	...
	(経緯と切り分けは KCMConstants.h の kKCMCursorSettleMillis を参照)。★対で呼ばないと
...
public:
	...
	virtual bool16 BeginTracking(IEvent* theEvent)
	{
		...
		// ジェスチャ分類は KCMClassifyGesture の1本に集約(独立の修飾キー判定を書かない。KCMPeek.h)。
		// ★Mac 対応(2026-07-25 追補): MacCtrlDown() も渡す。macOS の Control+クリックは副ボタンの標準
		//   ジェスチャなので KCMClassifyGesture 側で「未割当」に倒す。Windows では常に kFalse。
		...
		//     待ち = kKCMCursorSettleMillis(KCMConstants.h)。ゴミがまだ出るならその値を増やす。
		...
		// (InterfacePtr(p, iid) 側は p==nil を許す。InterfacePtr.h:459。2026-07-25 追補 に KCM 全体で統一)。
		...
		{
			// ★Hide/Show の対は KCMCursorHideGuard に任せる(2026-07-30 の監査で RAII 化)。Show が走る
			...
			if (result)
			{
				// ★押下中 HUD を「出す」印を立てる。★KCMTrackerRevealBegin の**前**に立てること:
				//   Begin 自身が再描画を要求する(KCMTrackerHudInvalidate)ので、先に立てておけば
				...
				//     (KCMPeekGesture.cpp の KCMTrackerRevealBegin＝`KCMMouseIsOverTarget()`
				//      が偽なら早期 return する。★2026-08-17 に参照先を訂正＝旧記述の
				//      `KCMPeek.cpp:1841-1844` は分割前の行で、あのファイルは今 907 行しかない)。
				//     そのため Source 窓・Stop 中・第3の文書では HUD が1度も描かれなかった。以後 HUD は
				//     自分で再描画を要求する(理由の全文は KCMTrackerHud.cpp の Invalidate のコメント)。
				//   描画の実体は**UI 側の KCMUIDrawEvent.cpp**の2系統(帯の前面 / カンバス背景)。
				//   ⚠2026-08-18(不具合再検査 B-U2)訂正＝「KCMDrawEventHandler の2系統」は分割前の名。
	...
	virtual void ContinueTracking(const PBPMPoint& where, bool16 mouseDidMove)
	{
		...
		// Alt+左「色比較」中: 現在位置で CMYK を再サンプル(KCMTrackerUpdateCmykDrag 内で 50ms スロットル)
	...
	virtual bool16 EndTracking(IEvent* theEvent)
	{
		...
		// ★HUD の印は KCMTrackerRevealEnd の**前**に落とす(End 自身が再描画を要求するので、
	...
	virtual void AbortTracking(IEvent* theEvent)
	{
		...
		KCMTrackerHudEnd();			// EndTracking と同じ後始末(中断でも HUD を残さない)
		KCMTrackerRevealEnd();
	}
...
private:
	...
	void InstallCmykCursor()
	{
		...
		CursorSpec spec(GetPlugIn()->GetPluginID(), IDFile(),
		                ...
		                KCMTrackerCmykCursorProc(), kFalse /*同期描画=ゴミ無し*/);
