...
namespace KCMJa
{
	...
	const char16_t kHint[] =
		...
		u"・Start Comparison 済みの状態で登録・解除・トグル切替をすると、その場で自動的に比較し直します\n\n"
		u"【ページのチェック(Check)】\n"
		u"確認が済んだページにチェック印を付けられます。消すまで残ります(Stop Comparison でも消えず、文書を閉じたときに忘れます)。\n"
		...
		u"・登録(Added/Removed)は比較のための指定なので、文書には書き込まれず、Stop Comparison で消えます\n\n"
		u"【ページ比較の更新(Refresh Page Comparison)】\n"
		u"編集したページだけを比較し直せます。Start Comparison 中で、かつPixel比較モードのときだけ使用できます"
	...
	const char16_t kHint2[] =
		...
		u"文書の写しを、選んだ場所にファイルとして保存し、そのファイルを Source にします。"
		u"編集してから Start Comparison を押すと、その時点からの変更が3つのモードのどれでも見られます。\n"
		u"・使い方: パネルのフライアウトメニューから「Task Start」を選びます。"
		u"文書のあるフォルダーに、文書名と日時から作った名前で保存ダイアログが開くので、保存を押します。キャンセルすると何も起きません\n"
		u"・Source の行には写しのパスが出ます。写しは開きません。開くのは Start で、止めた後も開いたままです\n"
		u"・写しを取る文書: ターゲットを指定していればその文書、していなければ前面の文書です。その同じ文書がターゲットになります\n"
		u"・比較中でも押せます(その比較は先に止まります)\n"
		u"・写しはふつうの InDesign 文書です。消すまでそこに残り、開く・印刷する・比較する、どれも他の文書と同じようにできます\n\n"
		u"【Before/After PDF 報告書(Export Before/After PDF Report)】\n"
		u"比較中の結果を1つの PDF に3部構成で書き出します。1ページ目＝2つの文書の名前と、左 Before・右 After の見出し。"
		u"次に画素比較で変更のあったページごとに横長1ページ＝左に旧版(比較マーク入り)、右に新版(素のまま)。このページに文字は入れません。追加ページは左が空、削除ページは右が空です。"
		u"次に Story の表＝左に旧版の文、右に新版の文を変更1件ごとに1行。変わっていない部分は薄く、変わった部分は濃く出ます。ルビと圏点は本物のルビ・圏点で、脚注・文末脚注の番号は上付きで出ます。"
		u"最後に Resources の表＝定義ごとに、変わった属性を1行ずつ、左に旧値・右に新値。どちらの表もパネルと同じ列(ID または Kind と、記号 Δ)を持ち、長さの値には文書の単位を括弧で添えます(例: 8.5 (12 Q)。パネルでも同じ)。\n"
		u"・使い方: 比較中に、パネルのフライアウトメニューから「Export Before/After PDF Report」を選びます\n"
		u"・保存ダイアログで場所と名前を選びます(初期名は「<文書名>.compare-report.pdf」)。書き出したら PDF ビューアで開きます\n"
		u"・2つの表はどの比較モードでも出ます(そのモードで比べていないものは報告書のために比べます)。画素比較のページは Pixel モードで比べたときだけで、他のモードでは1ページ目にその旨が出て、追加・削除ページだけが並びます\n"
		u"・旧版側のマークは変わった箇所のリングだけで、ページの縁の枠は付けません\n"
		u"・現在の PDF 書き出し設定を使います(見開き・セキュリティは切ります)。Task Start が Source のときは左側が保存した写しの状態です\n\n"
		u"【ストーリー ID(Show Story IDs)】\n"
		...
		u"・画面では常に、ズームによらず同じ大きさで表示します。印刷・PDF には「Print comparison marks」が ON のときだけ出ます(Before/After PDF 報告書には After 側だけに出ます)\n"
		...
		u"ファイルではなく今の文書が決めます。Word で表の隣に改行を足しても消しても何も変わりません。表の隣で段落を割る・つなぐのは InDesign で行ってください\n\n"
		...
		u"・変更の行(ストーリーの下にぶら下がる行)を右クリックすると「Restore Source Text」が出ます。その1件だけを旧版の状態に戻します"
		u"(文字／ルビ＝変更・追加・削除のいずれも、モノルビはモノルビに・グループルビはグループルビに／圏点＝種類)。"
		u"Ctrl+Z 1回で元に戻り、戻したストーリーはすぐに比べ直されます。"
		u"比較の後にそのストーリーを編集していても置換できます(そのストーリーだけを比べ直してから書きます)。"
		u"ただし置き換わる文字そのものを編集していた場合は一度だけ止まり、今の姿を確かめてからもう一度押すと入ります。"
		u"圏点がカスタム文字の場合、本文とルビ・圏点が同じ段落で同時に変わっている場合(先に本文を戻してから)は、理由をステータス行に出して断ります\n"
		u"・ストーリーの行そのものを右クリックすると「Restore All in This Story」が出ます。"
		u"そのストーリーの変更を全部、1回の操作・1段の取り消しで戻します\n"
		u"・旧版の文字は、そのときの Source から読みます。比較の後に Source の文書を編集すると、"
		u"戻る中身が行に見えているものと違うことがあります(先に比べ直してください)\n\n"
		...
		u"・比較していなくても使え、Stop Comparison でも消えません(チェック印と同じ)\n"
