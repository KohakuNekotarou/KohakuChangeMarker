...
namespace KCMJa
{
	...
	const char16_t kHint2[] =
		...
		u"【Before/After 報告書(Export Before/After Report)】\n"
		u"比較中の結果を1つの PDF に書き出します。先頭に要約ページ(2つの文書・モード・件数・Story Edits の行・Resources の要約)、"
		u"続けて変更のあったページごとに横長1ページ＝左に旧版(比較マーク入り)、右に新版(素のまま)。追加ページは左が空、削除ページは右が空です。変更ページだけを出します。\n"
		u"・使い方: 比較中に、パネルのフライアウトメニューから「Export Before/After Report」を選びます\n"
		...
		u"【フレームとストーリーの UID(Show Frame UIDs)】\n"
		u"テキストを持つフレーム(テキストフレーム・パス上テキスト・アンカー付きフレーム)の左上に、そのフレームの内部 ID と、載っているストーリーの ID を表示します。"
		u"「ID:」はスクリプトの textFrame.id、「Story:」は story.id で、Story Edits 一覧の ID 列と同じ番号です。\n"
		u"・使い方: パネルのフライアウトメニューの「Show Frame UIDs」を ON にします(既定は OFF。パネル設定に保存されます)\n"
		u"・画面では常に、ズームによらず同じ大きさで表示します。印刷・PDF には「Print comparison marks」が ON のときだけ出ます\n"
		u"・パスにもテキストが載っているフレームは Story の行が2つになります\n\n"
