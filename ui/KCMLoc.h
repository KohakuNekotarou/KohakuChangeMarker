...
namespace KCMJa
{
	...
	const char16_t kHint2[] =
		...
		u"編集してから Start Comparison(または Refresh Comparison)を押すと、その時点からの変更が3つのモードのどれでも見られます。\n"
		...
		u"・現在の PDF 書き出し設定を使います(見開き・セキュリティは切ります)。Task Start が Source のときは左側が記録した時点の状態です\n\n"
		...
		u"・まとめて戻すこともできます。ストーリーの行(親の行)を右クリックすると「Restore All in This Story」＝そのストーリーの変更を全部、"
		u"フライアウトの「Restore All Stories」＝変更のある全ストーリーを全部。どちらも Ctrl+Z 1回で戻ります。"
		u"入れられない変更(カスタム圏点など)はそこで止まらずに飛ばし、「何件入れて何件飛ばしたか」をステータス行に出します。"
		u"★全ストーリーのほうは、書き込む前に件数を出して確認します(ストーリー単位のほうは確認しません)\n\n"
