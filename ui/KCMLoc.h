...
namespace KCMJa
{
	...
	const char16_t kHint2[] =
		...
		u"・変更の行(ストーリーの下にぶら下がる行)を右クリックすると2つの項目が出ます。「Copy Source Text」は旧版側の文字(ルビならその読み)をクリップボードへ。"
		u"「Restore Source Text」はその1件だけを旧版の状態に戻します(文字／ルビ＝変更・追加・削除のいずれも、モノルビはモノルビに・グループルビはグループルビに／圏点＝種類)。"
		u"Ctrl+Z 1回で元に戻り、戻したストーリーはすぐに比べ直されます。"
		u"比較の後にそのストーリーを編集していた場合(先に行の「Refresh Story Comparison」を)、圏点がカスタム文字の場合、本文とルビ・圏点が同じ段落で同時に変わっている場合(先に本文を戻してから)は、理由をステータス行に出して断ります\n\n"
