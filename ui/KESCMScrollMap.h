...
//   だけ中身が変わる(そのマスターのページだけを載せる)が、スプレッドの切り替えも KESCM のどの
...
//     ①KESCMActionComponent(フライアウトのトグル) … ON なら Attach＋Invalidate / OFF なら DetachAll
//     ②KESCMPanelState(保存済みパネル設定の復元)   … **旗だけ**。
//   ⚠②が①と違って何もしないのは**正しい**(2026-08-19・不具合再検査 B-U8 で確認)＝
//   KESCMLoadPanelStateIfPresent は `sLoaded` の一度きりで、走るのは UI 側 Startup。**その時点では
