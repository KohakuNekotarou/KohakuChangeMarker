...
//
//  ★**インクルードガードは相方(source/KESCMLoc.h)とわざと別にしてある**（こちらは `__KCMUILoc_h__`。
...
//  ⚠`KESCMBoundaryID.h` は逆で、**同一内容のコピー**（同日実測＝1行も違わない）なのでガード共有が正しい。
//
//========================================================================================
...
namespace KESCMJa
{
	...
	// ★How to Use 本文の**2本目**(2026-08-19)＝ブック比較の節と免責文。上の kHint の後ろへ
	//   KESCMActionComponent::DoUsage が連結して1つのアラートに出す＝**読み手からは1本の文章**。
	//   ⚠**分けた理由は英語側の都合**＝odfrc は StringTable の1文字列に長さ上限があり、
	//     enUS の kKESCMHintKey は残り約100B しか無かった([[odfrc-long-string-limit]])。
	//     **こちら(日本語)は u"" リテラルなので上限は無い**が、**両言語で同じ構造にしてある**
	//     ---- 対になるキーが片方だけ存在すると、次に足す人がどちらへ足すか迷う。
	//   ★免責文はここの末尾に置く(1本目の末尾ではない)。単純に後ろへ繋ぐと
	//     「免責 → ブックの説明」の順になって不自然なため。**次に本文を足すのもこちら側。**
	const char16_t kHintBook[] =
		u"\n【ブックの比較(Compare Books)】\n"
		u"ブック(.indb)どうしを章(ドキュメント)単位で比べ、どの章が変わったかを一覧します。"
		u"文書の比較(Start)とは独立していて、比較を開始したり枠を消したりはしません。\n"
		...
		u"・この比較が開いた章は終わったら閉じます。もともと開いていた章はそのままにします\n\n"
