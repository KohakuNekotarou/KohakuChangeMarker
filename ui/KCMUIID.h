...
DECLARE_PMID(kActionIDSpace, kKESCMPopupShowSrcMarksActionID, kKCMUIPrefix + 10)	// パネルのフライアウトの「Always Show Marks on Source」チェック式トグル(Source側にも枠を常時表示。OPPでも表示・印刷にも出す)。★既定 OFF で Start は触らない(2026-08-22 変更＝設定はパネル設定に保存され起動時に復元されるので、Start が上書きすると保存した選択が消える)
...
DECLARE_PMID(kActionIDSpace, kKESCMPageCheckToggleActionID, kKCMUIPrefix + 24)	// ページパネルのページ右クリック(RtMenuPagesPanel)のトグル「KCM: Check」(選択ページに✓印を付け外し。Start中限定・Stopで消去。チェック/有効無効は kCustomEnabling。★**付けられるページはモードで違う**=Pixel はマーク付きのみ/Story は全ページ、2026-08-24。答えは model の KESCMCollectCheckablePageUIDs 1本。実体 KESCMPageCheck.cpp、✓描画は KESCMDrawEventHandler の isThumb 分岐)
...
DECLARE_PMID(kActionIDSpace, kKESCMPageRefreshCompareActionID, kKCMUIPrefix + 30)	// ページパネルのページ右クリック(RtMenuPagesPanel)の実行アクション「KCM: Refresh Page Comparison」(選択ページの比較を再検出して枠/サムネイルを更新。旧 Ctrl+ミドルのスプレッド再比較を移設。Start中・★**Pixel モード**・前面文書が Target のときだけ有効=それ以外は項目ごと消える。kCustomEnabling。★Story モードで出さない理由=あちらはページを1枚もラスタ化しないので押しても画面が変わらない/Story の更新は行の右クリック kKESCMStoryRowRefreshActionID が持つ、2026-08-24。実体 KESCMPeek.cpp の KESCMRefreshComparisonForSelectedPages)
...
DECLARE_PMID(kActionIDSpace, kKESCMPopupShowTgtMarksActionID, kKCMUIPrefix + 45)	// ★パネルのフライアウトの「Always Show Marks on Target」チェック式トグル(2026-08-22 ユーザー要望「ツールでボタンを押さなくても常にマークが出る様に、それをピクセルの方もストーリーの方にも」)。Source 版(+10)と対で、ON の間は Target 文書のマークを画面に常時表示する。★Pixel では比較リング(sTgtMarksOn→alwaysScreen)、Story では変更箇所に敷く色地(ui/KESCMStoryPressMarks.cpp → model の KESCMStoryMarkBuild)＝同じトグルが両モードで意味を持つ。⚠画面のみ＝印刷/PDF は「Print comparison marks」が単独で決める(Source 版が印刷にも出るのとは非対称・意図的)。★既定 OFF で Start は触らない(Source 版と同じ＝設定はパネル設定に保存され、起動時に復元される)
...
#define kKESCMShowSrcMarksMenuKey	kKESCMStringPrefix "kKESCMShowSrcMarksMenuKey"	// パネルのフライアウト「Always Show Marks on Source」トグルのメニュー名
#define kKESCMShowTgtMarksMenuKey	kKESCMStringPrefix "kKESCMShowTgtMarksMenuKey"	// パネルのフライアウト「Always Show Marks on Target」トグルのメニュー名
...
#define kKESCMStoryRowRefreshMenuKey	kKESCMStringPrefix "kKESCMStoryRowRefreshMenuKey"	// Story Edits の行の右クリックメニューの「Refresh Story Comparison」項目名
// Story Edits の行の右クリックメニュー(2026-08-21)。上の章行メニューと同じ機構＝この名前の MenuDef
// サブツリーを KESCMStoryRowEH::RButtonDn が HandlePopupMenu でカーソル位置に出す。
// ★根の名前は画面に出ないので、章行と同じく翻訳キーではなく素のリテラルでよい。
#define kKESCMStoryRowMenuName		"KESCMRtMenuStoryRow"
...
// ★How to Use 本文の**2本目**(2026-08-19)。DoUsage が kKESCMHintKey の後ろへ連結して1つの
...
//   ⚠**日本語側(ui/KESCMLoc.h の kHint / kHint2)も同じ位置で割ってある。** 片方だけ動かさないこと。
#define kKESCMHint2Key			kKESCMStringPrefix "kKESCMHint2Key"
...
#define kKESCMShowTgtMarksMenuItemPosition	9.29	// チェック式トグル「Always Show Marks on Target」(★Source 版 9.30 の直上＝新旧の並びと同じ順序で読める)
#define kKESCMShowSrcMarksMenuItemPosition	9.30	// チェック式トグル「Always Show Marks on Source」
...
#define kKESCMStoryRowRefreshMenuItemPosition	1.0	// Story Edits 行の右クリック「Refresh Story Comparison」(別のサブツリーなので章行と同じ 1.0 でよい)
// ── 情報系(末尾) ──
