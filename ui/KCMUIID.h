...
DECLARE_PMID(kActionIDSpace, kKCMPopupExportReportActionID, kKCMUIPrefix + 65)	// ★"Export Before/After Report" on the panel flyout (a plain command, 2026-09-13, the user's pick): one PDF, a summary page then every changed page with the older version on the left and the newer (rings printed) on the right, written next to the Target. Live only while a comparison is running. Facade ExportBeforeAfterReport; the work is KCMReport.cpp
...
DECLARE_PMID(kActionIDSpace, kKCMPopupShowFrameUidsActionID, kKCMUIPrefix + 67)	// ★"Show Frame UIDs" check toggle on the panel flyout (2026-09-13, the user's ask: the story's UID on the layout, found from the story and put on the item that holds it). ON = every page item holding a story is labelled at its top left with "ID:<its UID>" and "Story:<the story's UID>" - a text frame, text on a path, an anchored frame alike; on screen always, in print and PDF with "Print comparison marks" on. Default OFF. The model draws it from the same global adornment as the rings (KCMRingAdornment.cpp §1.5); facade Get/SetShowFrameUids; saved with the panel settings ("showFrameUids")
...
#define kKCMExportReportMenuKey	kKCMStringPrefix "kKCMExportReportMenuKey"		// the menu name of "Export Before/After Report" on the panel flyout
#define kKCMShowFrameUidsMenuKey	kKCMStringPrefix "kKCMShowFrameUidsMenuKey"		// the menu name of the "Show Frame UIDs" toggle on the panel flyout
...
#define kKCMShowFrameUidsMenuItemPosition	9.285	// check toggle "Show Frame UIDs", directly under the page-number badge: both put a number on the page that is not part of the page
...
#define kKCMExportReportMenuItemPosition	9.535	// plain command "Export Before/After Report" (the changed pages side by side as one PDF), directly below the TSV export it complements
