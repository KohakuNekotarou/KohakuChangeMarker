...
DECLARE_PMID(kActionIDSpace, kKCMPopupOpenOriginIdmlActionID, kKCMUIPrefix + 80)	// ★"Open Task Start as IDML" on the panel flyout, right under Task Start (a plain command, 2026-09-20, the user: "a menu item that puts the Task Start state back as a document that is not hidden"). The held origin is rehydrated THE WAY THE COMPARISON DOES IT (the same internal IDML, the same injection, the same ImportINX) and opened in a window, with the sacrificial paragraphs LEFT IN so that what the import ate can be read off the document. ★**Live whenever an origin is held** (facade HasOrigin). ⚠**It is a measuring instrument as much as a convenience**: it IS the comparison’s own copy, so a nested table that vanishes there vanishes here, in front of the reader. Facade OpenOriginAsIdml; the work is KCMOriginIdml.cpp
