...
DECLARE_PMID(kActionIDSpace, kKCMPopupExportStoryTextActionID, kKCMUIPrefix + 70)	// ★"Export Story Text..." on the panel flyout (a plain command, 2026-09-15), directly under Task Start: every story of the active document is written as one HTML file in a dated folder under one the reader picks, for editing outside InDesign and importing again. **It only reads** - nothing is written into the document, and the walk is wrapped in IDataBase::SaveRestoreModifiedState so it is not even dirtied. ★Live whenever there is an active document (facade GetActiveDocDB). ⚠It borrowed CanTakeTaskStart for this until 2026-09-21, when the two stopped being the same question - Task Start copies the chosen Target when there is one. Facade ExportStoryText; the work is KCMStoryTextExport.cpp and the format is KCMStoryHtml.cpp (pure functions, tested outside InDesign in work/kcm-storyhtml-test)
DECLARE_PMID(kActionIDSpace, kKCMPopupOpenOriginIdmlActionID, kKCMUIPrefix + 80)	// ★"Open Task Start as IDML" on the panel flyout, right under Task Start (a plain command, 2026-09-20, the user: "a menu item that puts the Task Start state back as a document that is not hidden"). The held internal IDML is handed to ImportINX ★**UNTOUCHED** (the same day, the user: "without the sacrificial text - just make a document out of it doing nothing to it") and the result is opened in a window: no injection, and nothing written into the copy afterwards, so what the reader sees is the import's own answer. ⚠**It was the comparison's own copy for one evening** (same injection, sacrificial paragraphs left in) - that is no longer what it shows. ★**Live whenever an origin is held** (facade HasOrigin). ⚠**It is a measuring instrument as much as a convenience**: what the import drops, and what it does to a nested table, is visible in front of the reader. Facade OpenOriginAsIdml; the work is KCMRehydrate.cpp
DECLARE_PMID(kActionIDSpace, kKCMPopupImportStoryTextActionID, kKCMUIPrefix + 71)	// ★"Import Story Text..." on the panel flyout (a plain command, 2026-09-15), under Export: the reader picks the edited story FILES (a "<decimal>.html", or a .docx written by Export Story Text as Word...), a Task Start copy of the document is saved to a file THEY pick, the edited words go into the DOCUMENT, and the Story comparison starts against that copy. ★★★**THE DOCUMENT IS CHANGED BY THIS, AND EVERYTHING GOES IN AT ONCE** (2026-09-19, the reader's decision: overset then shows itself on the real page); "Undo the Restore" sends one change back. ⚠**The old note here - "the document is not changed by this" - was the 2026-09-15 design and is measured false.** ★A cancelled save dialog ends the import, in silence. ★Live with an active document and no comparison running (⚠unlike Task Start, it does NOT stop one and take over). Facade ImportStoryText; the work is KCMStoryTextImport.cpp, and the copy is KCMTakeTaskStartCopy (KCMTaskStartSave.cpp) - the same one the flyout's Task Start takes
...
#define kKCMOpenOriginIdmlMenuKey	kKCMStringPrefix "kKCMOpenOriginIdmlMenuKey"	// ★the menu name of "Open Task Start as IDML" - the held origin opened as an untitled document (2026-09-20). ⚠**IDML is in the name on purpose** (the user asked for it): the reader should be able to tell that what opens is the origin's own format, not a re-made document
...
#define kKCMOpenOriginIdmlMenuItemPosition	9.0205	// "Open Task Start as IDML": directly under Task Start, because it is about the origin that item takes and about nothing else
