...
DECLARE_PMID(kWidgetIDSpace, kKCMStoryRubyKindWidgetID, kKCMUIPrefix + 53)	// ★ruby change row, right, UPPER LINE: how the ruby is SET -- "Mono" or "Group" (user's request, 2026-09-08: "when it changes from group to mono the row does not say what changed"). ⚠**Only the ruby row has it**: it fills the half of the right-hand column that the sign leaves empty, and an ordinary change row has no upper line to fill. ⇒ the widget manager must find it before writing, rather than going through SetNodeName, which asserts on a row that does not carry it
